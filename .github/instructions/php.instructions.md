...
- Compact `if` / `elseif`: keep a void `return;`, `continue;`, or `break;` on the same line when that line is at most 80 columns (tab = 4). Put a `return` with a value on the next line, unbraced. If that line still exceeds 80 columns or must wrap for operators or calls, wrap the condition and use braces. Do not wrap only the `return`.
