...
- When outside global namespace:
	...
	6. When importing multiple symbols from the same namespace, use a single import statement with a comma-separated list and each item on a new line.
	7. Import only first-order names (one identifier after the namespace prefix). Do not import nested paths. Qualify the rest at the call site: `use The_SEO_Framework\{ Admin, Data };` then `Admin\SEOBar\Builder`, not `use The_SEO_Framework\{ Admin\SEOBar\Builder, Data };`.
	...
	9. Do not namespace-escape calls to functions defined in the current namespace unless phpcs.xml already requires an exception.
- Repo-wide rewrite of existing `use` lists (nested paths, sort) is on Hold until the user says **go**. Apply first-order names and alphabetical order to new `use` lists.
