...
window.tsfPTGB = function() {
	...
	const _initPrimaryTerm = () => {
		...
		function primaryTermSelector( { taxonomySlug, _legacySelectedTerms } ) {
			...
			// var_dump() can we remove this? How did we test for < 11.3? WP 5.9 should be 12.0+?
			// 11.3+ support: https://github.com/sybrew/the-seo-framework/commit/8b5836930f0bc33373074e8f15c1d4d46e1778d4
			// <11.3 support regress: https://github.com/sybrew/the-seo-framework/commit/c4dfc4fba64ba4aabdd304ce672dd1389ce5f47f
			useEffect(
				() => {
					// Legacy Gutenberg (<11.3.0) support.
					if ( _taxonomy?.rest_base && _legacySelectedTerms?.length && ! _legacyIsResolving ) {
						if (
							   ! _legacyAvailableTerms?.length
							   // Find differences in stored/selected ids.
							|| _legacySelectedTerms.filter(
								id => ! availableTerms.map( term => term.id ).includes( id )
							).length
						) {
