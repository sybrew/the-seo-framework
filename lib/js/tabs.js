...
window.tsfTabs = function() {
	...
	const toggleTo = ( stackId, target ) => {
		...
		const setCorrectTab = async () => {
			let newTab = document.getElementById( `${_toggleCache.get( 'target' ).get( cacheId )}-content` );

			// Lock height, so to prevent jumping.
			lockHeight();
			allContent.forEach( el => { el.classList.remove( stack.HTMLClasses.activeTabContent ) } );

			newTab.classList.add( stack.HTMLClasses.activeTabContent );
			unLockHeight();
			fadeIn( newTab );
		...
		const testTab = async () => {
			// Regain this value from a new query, for the toggle's target-cache might've cahnged.
			let toggledContent = document.getElementById( `${_toggleCache.get( 'target' )
				.get( cacheId )}-content` );

			// Test if the correct tab has been set--otherwise, try again.
			// Resolve if the query fails, so to prevent an infinite loop.
			if ( ! toggledContent || toggledContent.classList.contains( stack.HTMLClasses.activeTabContent ) ) {
				clearPromise();
