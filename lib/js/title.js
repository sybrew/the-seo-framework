...
window.tsfTitle = function( $ ) {
	...
	/**
	 * @since 4.0.0
	 * @since 4.1.0: Added 'useSocialTagline', boolean.
	 * @access private
	...
	let state = {
		...
		useSocialTagline:     l10n.states.useSocialTagline,
	 ...
	 * @since 4.0.0
	 * @since 4.0.6 Now changes behavior depending on RTL-status.
	 * @since 4.1.0 Now also sets references without the additions.
	 * @access private
	 *
	 * @function
	 * @param {!jQuery.Event} event
	...
	const _setReferenceTitle = ( event ) => {
		let reference   = document.getElementById( 'tsf-title-reference' ),
			referenceNa = document.getElementById( 'tsf-title-noadditions-reference' ),
			text        = state.allowReferenceChange && event.target.value || state.defaultTitle,
			textNa      = '';

		if ( ! reference || ! referenceNa ) return;
		...
		referenceNa.innerHTML = tsf.escapeString( tsf.decodeEntities( tsf.sDoubleSpace( textNa.trim() ) ) );
