...
window.tsfTitle = function( $ ) {
	 ...
	 * @type {String}
	 */
	const protectedPrefix = tsf.escapeString( l10n.params.protectedTitle );
	/**
	 * @since 4.1.0
	 * @access public
	 * @type {String}
	 */
	const privatePrefix = tsf.escapeString( l10n.params.privateTitle );
	...
	/**
	 * @since 4.1.0
	 * @internal Use getStateFor() instead.
	 * @access private
	 *
	 * @param {String} id
	 * @param {String} value
	...
	/**
	 * @since 4.1.0
	 * @internal Use updateStateFor() instead.
	 * @access private
	 *
	 * @param {String} id
	 * @param {String} value
	 ...
	 */
	const getState = part => {
		tsf.deprecatedFunc( 'tsfTitle.getState()', '4.1.0', 'tsfTitle.getStateFor()' );
		return getStateFor( getLegacyElementId(), part );
	}

	/**
	 * Returns state for ID.
	 *
	 * @since 4.1.0
	 * @access public
	 *
	 ...
	 * @return {(Object<string, *>)|*|null}
	 */
	const getStateFor = ( id, part ) => {
		// Backward compat.
		if ( 'useTagline' === part ) {
			tsf.deprecatedFunc( 'tsfTitle.getStateFor( id, \'useTagline\' )', '4.1.0', 'tsfTitle.getStateFor( id, \'addAdditions\' )' );
			part = 'addAdditions';
		}
	...
	const updateState = ( part, value ) => {
		tsf.deprecatedFunc( 'tsfTitle.updateState()', '4.1.0', 'tsfTitle.updateStateFor()' );
		return updateStateFor( getLegacyElementId(), part, value );
	}

	/**
	 * Updates state for ID.
	 *
	 * There's no need to escape the input, it may be double-escaped if you do so.
	 *
	 * @since 4.1.0
	 ...
	 * @return {undefined}
	 */
	const updateStateFor = ( id, part, value ) => {
		// Legacy was probably called, but doesn't exist (yet).
		if ( ! ( id in states ) ) return;

		// Backward compat.
		if ( 'useTagline' === part ) {
			tsf.deprecatedFunc( 'tsfTitle.updateStateFor( id, \'useTagline\' )', '4.1.0', 'tsfTitle.updateStateFor( id, \'addAdditions\' )' );
			part = 'addAdditions';
		}
	...
	/**
	 * Updates state for all elements.
	 *
	 * There's no need to escape the input, it may be double-escaped if you do so.
	 *
	 * @since 4.1.0
	...
	const updateStateAll = ( part, value ) => {
		titleInputInstances.forEach( element => {
			updateStateFor( element.id, part, value );
		} );
	}

	/**
	 * Returns title references for ID.
	 *
	 * @since 4.1.0
	 * @access public
	 *
	...
	/**
	 * Returns title references with no-additions (Na) for ID.
	 *
	 * @since 4.1.0
	 * @access public
	 *
	...
	const _setReferenceTitle = event => {
		let references           = _getTitleReferences( event.target.id ),
			...
			allowReferenceChange = getStateFor( event.target.id, 'allowReferenceChange' ),
			text                 = allowReferenceChange && event.target.value || getStateFor( event.target.id, 'defaultTitle' ) || '',
			textNa               = '';
		...
		if ( text.length < 1 || ! allowReferenceChange ) {
			// TODO Is this the best place to obtain it from?
			text = event.target.placeholder;
			// Don't affect the textNa value!
		} else {
			let prefix    = _getPrefixValue( event.target.id ),
				additions = _getAdditionsValue( event.target.id );

			if ( prefix.length ) {
				switch ( getStateFor( event.target.id, 'prefixPlacement' ) ) {
					case 'before':
			...
			if ( additions.length ) {
				switch ( getStateFor( event.target.id, 'additionPlacement' ) ) {
					case 'before':
						text = additions + text;
						break;
	...
	const _updateAdditionsValue = id => {
		...
		if ( getStateFor( id, 'addAdditions' ) ) {
			additionsValue = getStateFor( id, 'additionValue' );
			separator      = getStateFor( id, 'separator' );
		}

		if ( additionsValue ) {
			switch ( getStateFor( id, 'additionPlacement' ) ) {
				case 'before':
	...
	const _updatePrefixValue = id => {
		let value       = '',
			prefixValue = getStateFor( id, 'prefixValue' );

		if ( prefixValue ) {
			switch ( getStateFor( id, 'prefixPlacement' ) ) {
				case 'before':
					if ( tsf.l10n.states.isRTL ) {
						value = ' ' + prefixValue;
					} else {
	...
	const _updatePlaceholder = event => {
		...
		if ( ! getStateFor( event.target.id, 'allowReferenceChange' )
	...
	const _focusTitleInput = event => {
		...
		// TODO buffer? It now flickers... Can we even resolve this? What did I mean by "buffer"?
		input.focus();
		...
		switch ( event.detail ) {
			...
			case 2:
				let start, end;
				if (
					'additions' === type && 'after' === getStateFor( input.id, 'additionPlacement' )
			...
			default:
				...
				let length = 'additions' === type && 'after' === getStateFor( input.id, 'additionPlacement' ) ? inputValue.length : 0;
	...
	const _initAllTitleActions = () => {
		...
		document.addEventListener( 'wp-window-resized', _doResize );
	...
	const _loadTitleActions = titleInput => {
		...
		_getHoverPrefixElement( titleInput.id ).addEventListener( 'click', _focusTitleInput );
		_getHoverAdditionsElement( titleInput.id ).addEventListener( 'click', _focusTitleInput );
