...
window.tsfTitle = function( $ ) {
	...
	const _setReferenceTitle = event => {
		...
		let allowReferenceChange = getStateFor( event.target.id, 'allowReferenceChange' ),
	...
	const _updateAdditionsValue = id => {
		...
		if ( getStateFor( id, 'addAdditions' ) ) {
			additionsValue = tsf.escapeString( tsf.decodeEntities( getStateFor( id, 'additionValue' ) ) );
