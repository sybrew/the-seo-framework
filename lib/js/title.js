...
window.tsfTitle = function( $ ) {
	...
	let state = {
		...
		additionValue:        l10n.states.additionValue,
		prefixValue:          '',
		defaultTitle:         l10n.states.defaultTitle,
	};
	...
	const updateAdditionsValue = () => {
		...
		let value = '',
			additionsValue = state.useTagline && tsf.escapeString( tsf.sDoubleSpace( state.additionValue.trim() ) ) || '';
	...
	const updatePrefixValue = () => {
		let value       = '',
			prefixValue = tsf.escapeString( tsf.sDoubleSpace( state.prefixValue ) );
	...
	const _updatePlaceholder = ( event ) => {
		...
		let _placeholder = state.defaultTitle.trim();

		if ( prefix.length ) {
			switch ( state.prefixPlacement ) {
				case 'before':
					_placeholder = prefix + _placeholder;
					break;

				case 'after':
					_placeholder = _placeholder + prefix;
					break;
			}
		}
		...
		if ( additions.length ) {
			switch ( state.additionPlacement ) {
				case 'before':
					_placeholder = additions + _placeholder;
					break;

				case 'after':
					_placeholder = _placeholder + additions;
					break;
			}
		}
	...
	const _loadTitleActions = () => {
		...
		if ( ! titleInput instanceof Element ) {
			console.log( 'tsfTitle: Doing it wrong! Register the title input at "tsf-onload". Debug me.' );
			return;
		}
