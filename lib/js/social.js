...
window.tsfSocial = function( $ ) {
	 ...
	 * @since 4.0.0
	 * @access public
	 * @todo deprecate; either remove, or convert to getStateFor. See tsfTitle & tsfDescription.
	 *
	 ...
	 * @since 4.0.0
	 * @access public
	 * @todo deprecate; either remove, or convert to updateStateFor. See tsfTitle & tsfDescription.
	 *
			...
			switch ( what ) {
				...
				case 'ref':
					if ( ! val.length ) {
						...
						if ( tsfTitle.getStateFor( inputs.meta.id, 'useSocialTagline' ) ) {
							val = refTitleValue;
						} else {
							val = refTitleNaValue;
						}
