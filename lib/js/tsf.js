...
window.tsf = {
	...
	_initPrimaryTerm: function() {
		...
		if ( ! tsf.hasInput || ! tsf.states.taxonomies )
			return;

		let termSelector = document.createElement( 'button' );
		termSelector.classList.add( 'tsf-primary-term-selector', 'tsf-set-primary-term' );
		termSelector.setAttribute( 'type', 'button' );
		termSelector.setAttribute( 'title', tsf.i18n.makePrimary );
		termSelector.setAttribute( 'aria-label', tsf.i18n.makePrimary );

		(function(){
			let circle = document.createElement( 'span' );
			circle.classList.add( 'tsf-primary-term-selector-circle' );
			termSelector.appendChild( circle );
		})();
		...
		const makePrimary = function( taxonomy, value ) {
			...
			$label = getInputWithVal( taxonomy, value ).closest( 'label' );
			$label.addClass( 'tsf-is-primary-term' );
			$label.find( '.tsf-set-primary-term' ).each( function( index, e ) {
				e.setAttribute( 'title', tsf.i18n.primary );
				e.setAttribute( 'aria-label', tsf.i18n.primary );
			} );
		...
		const unsetPrimaries = function( taxonomy ) {
			...
			$label.find( '.tsf-set-primary-term' ).each( function( index, e ) {
				e.setAttribute( 'title', tsf.i18n.makePrimary );
				e.setAttribute( 'aria-label', tsf.i18n.makePrimary );
			} );
		...
		const makeFirstPrimary = function( taxonomy ) {
			...
			if ( $checked.length ) makePrimary( taxonomy, $checked.val() );
		}
		const appendButton = function( taxonomy, element ) {
			...
			getInputWithVal( taxonomy, element.value ).each( function( index, e ) {
				...
				if ( ! $label.find( '.tsf-primary-term-selector' ).length )
					$label.append( termSelector.cloneNode( true ) );
			} );
		}
		...
		const initVars = function( taxonomy ) {
			...
			if ( ! $box.find( '.tsf-is-primary-term' ).length )
				makeFirstPrimary( taxonomy );
		}
		...
		const initActions = function( taxonomy ) {
			...
			jQuery( '#' + taxonomy + 'div' )
				.off( 'wpListAddEnd.' + ns )
				.on( 'wpListAddEnd.' + ns, '#' + taxonomy + 'checklist', updateList );
		}
		...
		const init = function() {
			tsf.states.taxonomies.forEach( function( taxonomy ) {
	...
	_initToolTips: function() {
		...
		const mouseEnter = function( event ) {
			...
			if ( desc && 0 === $item.find( 'div' ).length ) {
				...
				let $boundary = $item.closest( '.tsf-tooltip-boundary' );
				$boundary = $boundary.length && $boundary || jQuery( document.body );

				//= 9 = arrow (8) + shadow (1)
				let tooltipHeight = $item.outerHeight() + 9,
				...
				//* Because the tooltip is flex, it won't show its actual width and position. Text will.
				let $textWrap = $tooltip.find( '.tsf-tooltip-text-wrap' ),
					$text = $tooltip.find( '.tsf-tooltip-text' ),
					textWrapWidth = $textWrap.outerWidth( true ),
					textWidth = $text.outerWidth() + 4, // margin of painting error.
					tooltipWidth = $tooltip.outerWidth( true ),
					tooltipLeft = $textWrap.offset().left + textWidth,
					boundaryLeft = $boundary.offset().left + $boundary.outerWidth() - ( $boundary.prop( 'scrollLeft' ) || 0 );

				if ( tooltipWidth < textWidth && textWrapWidth > textWidth ) {
					let flexVal = '1 0 ' + ( textWidth ) + 'px';
					$textWrap.css(
						{ 'flex': flexVal },
						{ '-webkit-flex': flexVal }
					);
				...
				if ( tooltipLeft > boundaryLeft ) {
					//= Add indent relative to boundary. Add 2 to appeal to the eye.
					let horIndent = boundaryLeft - tooltipLeft - tooltipWidth - 2;
					$tooltip.css( 'left', horIndent + 'px' );
					...
					if ( $wrap.width() < 42 ) {
						...
						$tooltip.data( 'overflowDir', ( tsf.states.isRTL ? 'right' : 'left' ) );
					}
				}
			}
		}
		const mouseMove = function( event ) {
			...
			overflow = isNaN( overflow ) ? 0 : - Math.floor( overflow );

			if ( 'right' === overflowDir )
				overflow = - overflow;

			if ( overflow ) {
				$arrow.css( 'left', ( overflow ) + "px" );
			} else if ( mousex <= 8 ) {
				//* Overflown left.
				//= 6 is half of arrow, 2 px shadow margin.
				$arrow.css( 'left', 8 + "px" );
			} else if ( $tooltip.offset() !== void 0 ) {
				let $wrap = $tooltip.find( '.tsf-tooltip-text-wrap' ),
					width = $wrap.width() + overflow,
