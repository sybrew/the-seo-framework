	...
	_initToolTips: function() {
			...
			if ( desc && 0 === $hoverItem.find( 'div' ).length ) {
				...
				if ( textRight > boundaryRight ) {
					...
					if ( start < 0 ) {
						//= Don't overflow left.
						start = 0;
					} else if ( start + textWrapWidth > hoverItemWrapWidth ) {
						//= Don't overflow right.
						start = hoverItemWrapWidth - textWrapWidth;
					}
		...
		const mouseMove = function( event ) {
			let $target = jQuery( event.target ),
				...
				arrowBoundary = 7,
				mousex = pagex - $target.closest( '.tsf-tooltip-wrap' ).offset().left - arrowBoundary,
			...
			if ( overflow ) {
				...
				let $textWrap = $tooltip.find( '.tsf-tooltip-text-wrap' ),
					...
					minOffset = tooltipLeft - arrowBoundary * 2,
					maxOffset = tooltipRight - arrowBoundary * 2,
				...
				if ( pagex >= maxOffset ) {
					//= Overflow right.
					$arrow.css( 'left', textWidth - arrowBoundary * 3 + "px" );
				} else if ( pagex <= minOffset ) {
					//= Overflow left fallback.
					$arrow.css( 'left', arrowBoundary + "px" );
				} else {
