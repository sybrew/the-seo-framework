...
/**
 * The more you know 🌈⭐ We only use plugin "babel/plugin-transform-es2015-arrow-functions".
...
window.tsf = {
	...
	updatePixelCounter: function( test ) {
		...
		if ( ! testWidth ) {
			newClass = classes.bad;
			newWidth = '100%'; // It's 100% bad, not 0%.
		...
		const updateVisibility = function( event ) {
			...
			switch ( value ) {
				case 'password' :
					...
					// It won't invoke WordPress' password protection. TODO FIXME: file bug report.
	...
	_initSocialTitleInputs: function() {
		...
		const updateOgTitle = ( event ) => {
			...
			if ( ! ogLocked ) {
				...
				ogTitleValue = text.length ? tsf.sDoubleSpace( text ) : event.target.placeholder;
			}
			setPlaceholders();
			updateAllCounters();
		};
		const updateTwTitle = ( event ) => {
			...
			if ( ! twLocked ) {
				let text = event.target.value.trim();
				twTitleValue = text.length ? tsf.sDoubleSpace( text ) : event.target.placeholder;
			}
			setPlaceholders();
			updateAllCounters();
		};
	...
	_initSocialDescInputs: function() {
		...
		const updateAllCounters = () => {
			clearTimeout( updateAllCountersBuffer );
			updateAllCountersBuffer = setTimeout( () => {
				$ogDesc.each( ( i, el ) => updateCounter( el, getActiveValue( 'og' ), 'opengraph' ) );
				$twDesc.each( ( i, el ) => updateCounter( el, getActiveValue( 'twitter' ), 'twitter'  ) );
			}, 10 );
		};
		$reference.on( 'change', () => {
			referenceValue = $reference.text();
		...
		const updateOgDesc = ( event ) => {
			...
			if ( ! ogLocked ) {
				let text = event.target.value.trim();
				ogDescValue = text.length ? tsf.sDoubleSpace( text ) : event.target.placeholder;
			}
			setPlaceholders();
			updateAllCounters();
		};
		const updateTwDesc = ( event ) => {
			...
			if ( ! twLocked ) {
				let text = event.target.value.trim();
				twDescValue = text.length ? tsf.sDoubleSpace( text ) : event.target.placeholder;
			}
			setPlaceholders();
			updateAllCounters();
		};
