...
window.tsf = {
	...
	updateCharacterCountDescription: function( event ) {
		...
		if ( 0 === calcLength ) {
			...
			calcLength = event.target.placeholder && event.target.placeholder.length || 0;
		}
	...
	_initTitleInputs: function() {
		 ...
		 * @param {string} str
		 */
		const escapeSring = function( str ) {
		...
		let hoverPrefixPlacement,
			...
			hoverAdditionsValue = '',
			homeHasTagline = tsf.states.titleTagline,
		...
		const setHoverAdditionsValue = function() {
			...
			if ( tsf.states.isSettingsPage ) {
				...
				if ( tsf.params.titleAdditions.length )
					newValue = tsf.params.titleAdditions;
			}

			if ( newValue.length ) {
				newValue = escapeSring( newValue );
		...
		const setHoverPrefixValue = function() {
			...
			// TODO listen to post type settings... etc...
		...
		const updateHoverPlacement = function( event ) {
			...
			//= @TODO move this check somewhere more appropriate, before this method can even be called.
			...
			if ( ! hoverAdditionsValue.length && ! hoverPrefixValue.length ) {
				//= Both items are emptied through settings.
				$target.css( 'text-indent', 'initial' );
				return;
			}
			...
			let offsetPosition = isRTL ? 'right' : 'left',
				...
				verticalPadding = ( $target.outerHeight( true ) - $target.height() ) / 2,
				horizontalPadding = ( outerWidth - $target.innerWidth() ) / 2,
				leftOffset = ( $target.outerWidth( true ) - $target.width() ) / 2,
				textWidth = 0,
				maxWidth = 0;
			...
			(function() {
				...
				$offsetTest.css({
					...
					'letterSpacing' : $target.css( "letterSpacing" ),
					'fontSize' : $target.css( "fontSize" ),
				});
				textWidth = $offsetTest.width();
			})();

			maxWidth = $target.width() - horizontalPadding - textWidth;
			maxWidth = maxWidth < 0 ? 0 : maxWidth;

			let $additionsElement = jQuery( hoverAdditionsElement );
			if ( $additionsElement.length ) {
			...
			if ( outerWidth > leftOffset ) {
				switch ( hoverAdditionsPlacement ) {
					case 'before' :
						let indent = horizontalPadding + $additionsElement.width();

						if ( indent < 0 )
							indent = 0;

						$additionsElement.css( offsetPosition, leftOffset + "px" );
						$target.css( 'text-indent', indent + "px" );
						break;

					case 'after' :
						$additionsElement.css( offsetPosition, horizontalPadding + leftOffset + textWidth + "px" );
						break;
				}
			}
		}
		const updateCounter = function( event ) {
			...
			if ( target.value.length < 1 ) {
				titLen = getStringLength( target.placeholder );
			} else {
				titLen = ( hoverPrefixValue ? getStringLength( hoverPrefixValue ) : 0 )
				       + getStringLength( target.value )
				       + ( hoverAdditionsValue ? getStringLength( hoverAdditionsValue ) : 0 );
			}

			if ( titLen < 25 || titLen >= 75 ) {
				classNames += 'tsf-count-bad';
				counterName = tsf.getCounterName( 'bad' );
			} else if ( titLen < 42 || titLen > 55 ) {
				classNames += 'tsf-count-okay';
				counterName = tsf.getCounterName( 'okay' );
			} else {
				classNames += 'tsf-count-good';
				counterName = tsf.getCounterName( 'good' );
			}

			if ( tsf.additionsClass )
				classNames += ' ' + tsf.additionsClass;
			...
			return jQuery( counter ).text( output ).removeClass().addClass( classNames );
		}
		 ...
		 * @return {undefined}
		 */
		const updateTitles = function( event ) {
			updateHoverPlacement( event );
			updateCounter( event );
		}
		$doctitles.on( 'input', updateTitles );
		...
		const changeHomePageAdditionsVisibility = function( event ) {
			let prevHomeHasTagline = homeHasTagline;
			...
			if ( jQuery( event.target ).is( ':checked' ) ) {
				...
				homeHasTagline = false;
			}

			if ( prevHomeHasTagline ^ homeHasTagline ) {
				setHoverAdditionsValue();
				triggerInput();
			}
		}
		...
		const updateSeparator = function( event ) {
			...
			//* Update cache and fetch new separator.
		...
		const triggerUnregisteredInputOnResize = function() {
			let resizeTimeout = 0, prevWidth = 0;
		...
		const adjustHomepageTaglineExampleOutput = function( event ) {
			...
			if ( 0 === val.length ) {
				...
				if ( 0 === tsf.params.blogDescription.length ) {
					jQuery( '.tsf-sep-js' ).hide();
				} else {
					jQuery( '.tsf-sep-js' ).show();
				}
			} else {
				$tagline.text( val );
				jQuery( '.tsf-sep-js' ).show();
	...
	initToolTips: function() {
		...
		const loadToolTip = function( event ) {
			...
			if ( event.target.classList.contains( 'tsf-seo-bar-item' ) ) {
				isTouch && setTouchBuffer();

				mouseEnter( event );
				setEvents( event.target, isTouch );
			} else {
				let item = event.target.querySelector( ':hover' );

				//= Loop back to this method.
				if ( item ) {
					jQuery( item ).trigger( event.type );
				}

				//= Set at the end as it loops back.
				isTouch && setTouchBuffer();
			}
		...
		const setEvents = function( target, isTouch, unset ) {
			...
			if ( ! isTouch ) {
				if ( unset ) {
					jQuery( target ).off( 'mousemove mouseleave mouseout' );
				} else {
					jQuery( target ).on( {
						'mousemove'  : mouseMove,
			...
			//= Always set and reset touch events.
			let events = 'pointerdown.tsfTT touchstart.tsfTT click.tsfTT';
			jQuery( document.body ).off( events );
			if ( ! unset ) {
				jQuery( document.body ).on( events, touchRemove );
			}
		}
		const unsetEvents = function( target, isTouch ) {
			setEvents( target, isTouch, true );
		}
		const mouseEnter = function( event ) {
			...
			if ( desc && 0 === $item.find( 'div' ).length ) {
				$item.append( '<div class="tsf-explanation-desc">' + desc + '<div></div></div>' );

				let height = $item.find( 'div.tsf-explanation-desc' ).height() + 28;

				$item.find( 'div.tsf-explanation-desc' ).css( 'top', ( $item.position().top - height ) + 'px' );

				//= Initiate placement directly for Windows Touch.
				mouseMove( event );
			}
		}
		const mouseMove = function( event ) {
			let $target = jQuery( event.target ),
				...
				mousex = pagex - $target.closest( '.tsf-seo-bar-inner-wrap' ).offset().left - 12, // 24px width of arrow / 2 = 12 middle
				$balloon = $target.find( '.tsf-explanation-desc' ),
				$arrow = $balloon.find( 'div' );

			if ( mousex < 2 ) {
				//= 2 accounts for the shadow.
				$arrow.css( 'left', 2 + "px" );
			} else if ( $balloon.offset() !== void 0 ) {
				let width = $balloon.width(),
					maxOffset = $balloon.offset().left + width + 12;
		...
		const mouseLeave = function( event ) {
			...
			jQuery( event.target ).find( '.tsf-explanation-desc' ).remove();
		...
		const touchRemove = function( event ) {
			...
			let itemSelector = '.tsf-seo-bar-item',
				balloonSelector = '.tsf-explanation-desc';

			let $target = jQuery( event.target ),
				$keepBalloon;

			if ( $target.hasClass( 'tsf-seo-bar-item' ) ) {
		...
		const init = function() {
			...
			let $wrap = jQuery( '.tsf-seo-bar-inner-wrap' );

			$wrap.off( 'mouseenter pointerdown touchstart' );
			$wrap.on( 'mouseenter pointerdown touchstart', '.tsf-seo-bar-item', loadToolTip );
		}
		init();
	},
	...
	ready: function( $ ) {
		...
		$( '#tsf-description-separator input' ).on( 'click', tsf.separatorSwitchDesc );
