...
window.tsf = {
	 ...
	 *
	 * @const
	 * @type {Object<string, *>} states Localized strings
	 */
	 ...
	 *
	 * @const
	 * @type {Object<string, *>} params Localized strings
	 */
	...
	getCounterName: function( type ) {
		...
		//var name = tsf.i18n[ type ]; var_dump()
	...
	openImageEditor: function( event ) {
		...
		frame = wp.media( {
			button : {
				'text' : tsf.i18n['useThisImage'],
				'close' : false,
			},
			states: [
				new wp.media.controller.Library( {
					'title' : tsf.i18n['selectSocialImage'],
	...
	ready: function( jQ ) {
		...
		// jQ( '.tsf-js-confirm-reset' ).on( 'click.autodescription.autodescription_confirm_reset', tsf.confirmedReset ); var_dump()
		...
		// Image uploader
		jQ( '.tsf-set-social-image' ).on( 'click', tsf.openImageEditor );
	}
};
jQuery( tsf.ready );
