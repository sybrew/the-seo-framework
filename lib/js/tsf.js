	...
	_initTitleInputs: function() {
		...
		const updatePixels = function( event ) {
			...
			let test = {
				'e': pixels,
				'text' : text,
				'guideline' : tsf.params.titlePixels,
			};
		...
		jQuery( window ).on( 'tsf-flex-resize', triggerUnregisteredTitleChange );
	...
	_initDescInputs: function() {
		...
		const updatePixels = function( event ) {
			...
			let test = {
				'e': element,
				'text' : text,
				'guideline' : tsf.params.descriptionPixels
			}
	...
	_initToolTips: function() {
		...
		const mouseEnter = function( event ) {
			...
			if ( desc && 0 === $hoverItem.find( 'div' ).length ) {
				...
				let $hoverItemWrap = $hoverItem.closest( '.tsf-tooltip-wrap' ),
					...
					hoverItemWrap = $hoverItemWrap.width(),
					textWrapWidth = $textWrap.outerWidth( true ),
					textWidth = $innerText.outerWidth( true ),
					textRight = $textWrap.offset().left + textWidth,
					boundaryRight = $boundary.offset().left + $boundary.outerWidth() - ( $boundary.prop( 'scrollLeft' ) || 0 );

				let direction = ( tsf.states.isRTL ? 'right' : 'left' );

				if ( textRight > boundaryRight ) {
					//= Overflown over boundary (likely window)
					//= Add indent relative to boundary. 24px width of arrow / 2 = 12 middle
					let horIndent = boundaryRight - textRight - hoverItemWrap - 12;
					$tooltip.css( direction, horIndent + 'px' );
					$tooltip.data( 'overflow', horIndent );
					$tooltip.data( 'overflowDir', direction );
				} else if ( hoverItemWrap < 42 ) {
					...
					$tooltip.data( 'overflowDir', direction );
				} else if ( hoverItemWrap > textWrapWidth ) {
					...
					//= Centers according to left-hand side of tooltip and mouse position.
					let start = pagex - $hoverItemWrap.offset().left - ( textWrapWidth / 2 );

					if ( start < 0 ) {
						//= Don't overflow left.
						start = 0;
					} else if ( start + textWrapWidth > hoverItemWrap ) {
						//= Don't overflow right.
						start = hoverItemWrap - textWrapWidth;
					}

					$tooltip.css( direction, start + 'px' );
					$tooltip.data( 'adjust', start / 2 );
		...
		const mouseMove = function( event ) {
			let $target = jQuery( event.target ),
				...
				arrowMargin = 7, // 16px width of arrow / 2 = 8 middle - 1 for shadow adjustment
				mousex = pagex - $target.closest( '.tsf-tooltip-wrap' ).offset().left - arrowMargin,
			...
			if ( overflow ) {
			...
			} else if ( mousex <= arrowMargin * 2 ) {
				//* Overflown left.
				$arrow.css( 'left', arrowMargin + "px" );
			} else {
				let $textWrap = $tooltip.find( '.tsf-tooltip-text-wrap' ),
					textWidth = $textWrap.outerWidth( true ),
					tooltipLeft = $textWrap.offset().left,
					tooltipRight = tooltipLeft + textWidth,
					minOffset = tooltipLeft + arrowMargin * 2,
					maxOffset = tooltipRight - arrowMargin * 2,
					adjust = $tooltip.data( 'adjust' ),
					adjustDir = $tooltip.data( 'adjustDir' );

				//= Location aware indents.
				adjust = parseInt( adjust, 10 );
				adjust = isNaN( adjust ) ? 0 : - Math.round( adjust );

				if ( adjust ) {
					if ( 'left' === adjustDir ) {
						mousex = mousex + adjust * 2;
					} else {
						mousex = mousex - adjust * 2;
					}
				}
				...
				//= @TODO rtl?
				//= Arrows are relative to the tooltip. PageX is too.
				if ( pagex >= maxOffset ) {
					$arrow.css( 'left', textWidth - arrowMargin * 3 + "px" );
				} else if ( pagex <= minOffset ) {
					$arrow.css( 'left', arrowMargin + "px" );
				} else {
