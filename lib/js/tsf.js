...
window.tsf = {
	...
	_initTitleInputs: function() {
		...
		const setReferenceTitle = function( event ) {
			...
			if ( event.target.value.length < 1 || tsf.states.homeLocks.refTitleLock ) {
	...
	_initDescInputs: function() {
		...
		const updateCounter = function( event ) {
			...
			let descLen = tsf.getStringLength( tsf.unescapeString( reference.innerHTML ) ),
				counterClass = '',
				counterType = tsf.counterType,
				counterName = '',
				output = '';
		...
		const setReferenceDescription = function( event ) {
			...
			if ( event.target.value.length < 1 || tsf.states.homeLocks.refDescriptionLock ) {
				text = event.target.placeholder;
			} else {
				text = event.target.value;
			}
	...
	_initSocialTitleInputs: function() {
		...
		const setPlaceholders = () => {
			...
			// This escapes.
			ogLocked || ogPHLocked || $ogTitle.attr( 'placeholder', getActiveValue( 'reference' ) );
			twLocked || twPHLocked || $twTitle.attr( 'placeholder', getActiveValue( 'og' ) );
		...
		const updateCounter = ( target, value, threshold ) => {
			tsf.updateSocialCounter( target, tsf.escapeString( value ), threshold );
		};
		...
		const updateOgTitle = ( event ) => {
			...
			if ( ! ogLocked )
				ogTitleValue = event.target.value.length ? event.target.value : '';
			setPlaceholders();
			updateAllCounters();
		};
		const updateTwTitle = ( event ) => {
			if ( ie11killswitch ) return;
			if ( ! twLocked )
				twTitleValue = event.target.value.length ? event.target.value : '';
			setPlaceholders();
			updateAllCounters();
		};
	...
	_initSocialDescInputs: function() {
		...
		const setPlaceholders = () => {
			...
			// This escapes.
			ogLocked || ogPHLocked || $ogDesc.attr( 'placeholder', getActiveValue( 'reference' ) );
			twLocked || twPHLocked || $twDesc.attr( 'placeholder', getActiveValue( 'og' ) );
		...
		const updateOgDesc = ( event ) => {
			if ( ie11killswitch ) return;
			if ( ! ogLocked )
				ogDescValue = event.target.value.length ? event.target.value : '';
			setPlaceholders();
			updateAllCounters();
		};
		const updateTwDesc = ( event ) => {
			if ( ie11killswitch ) return;
			if ( ! twLocked )
				twDescValue = event.target.value.length ? event.target.value : '';
			setPlaceholders();
			updateAllCounters();
		};
	...
	attachUnsavedChangesListener: function() {
		...
		except = '.tsf-nav-tab-wrapper input, .tsf-flex-nav-tab-wrapper input'; // TODO change this.
		...
		jQuery( '.tsf-metaboxes input[type="submit"], #publishing-action input[type="submit"], #save-action input[type="submit"], a.submitdelete' ).click( () => {
