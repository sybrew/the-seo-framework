...
window.tsf = {
	...
	_initPrimaryTerm: function() {
		...
		let helpBox = document.createElement( 'span' );
		helpBox.classList.add( 'tsf-tooltip-wrap', 'tsf-primary-term-selector-help' );

		(function() {
			let question = document.createElement( 'span' );
			question.classList.add( 'tsf-tooltip-item' );
			question.innerHTML = '[?]';
			helpBox.appendChild( question );
		})();
		...
		const addHelp = function( taxonomy ) {
			...
			if ( ! $wrap.find( '.tsf-primary-term-selector-help' ).length ) {
				let help = helpBox.cloneNode( true );
				help.querySelector( '.tsf-tooltip-item' ).dataset.desc = taxonomies[ taxonomy ].i18n.help;
				$wrap.append( help );
			}
		}
