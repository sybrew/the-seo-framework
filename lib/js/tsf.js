...
window.tsf = function( $ ) {
	...
	const decodeEntities = ( str ) => {
		...
		if ( _canUseDOMParser() ) {
			...
			test.innerHTML = str;
			str = unescapeString( test.textContent );
		}
	 ...
	 * @since 3.3.0: 1. Now escapes all backslashes, instead of only double.
	...
	const unescapeString = ( str ) => {
		...
		unescapeStringRegex = unescapeStringRegex || new RegExp(
			Object.values( map ).map(
				// This doesn't do anything? Just escapes what already is escaped?
				// Odd I put this in; left for forward-compat.
				v => v.replace( /[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&' )
			).join( '|' ),
			'g'
		);
