	...
	_initToolTips: function() {
		...
		const mouseEnter = function( event ) {
			...
			if ( desc && 0 === $item.find( 'div' ).length ) {
				let $tooltip = jQuery( '<div class="tsf-tooltip"><div class="tsf-tooltip-text">' + desc + '</div><div class="tsf-tooltip-arrow"></div></div>' )

				$item.append( $tooltip );

				//= 9 = arrow (8) + shadow (1)
				let height = $tooltip.outerHeight() + 9;

				$tooltip.css( 'top', ( - height ) + 'px' );
				$tooltip.css( 'width', ( $item.width() ) + 'px' );
			}
		}
		const mouseMove = function( event ) {
			let $target = jQuery( event.target ),
				...
				$balloon = $target.find( '.tsf-tooltip' ),
				$arrow = $balloon.find( '.tsf-tooltip-arrow' );
		...
		const loadToolTip = function( event ) {
			...
			if ( event.target.classList.contains( 'tsf-tooltip-item' ) ) {
				...
				if ( item ) {
					if ( tsfL10n.states.debug ) console.log( 'Tooltip event warning: delagation' );
