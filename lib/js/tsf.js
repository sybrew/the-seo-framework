...
window.tsf = {
	...
	updatePixelCounter: function( test ) {
		...
		if ( testWidth > guideline ) {
			...
			counterWidth = ( guideline / ( testWidth + ( ( testWidth - guideline ) * 2 / 3 ) ) * 100 ) + '%';
			counterClass = 'tsf-pixel-counter-overflown';
		} else {
			//= Can never be over 100. Good.
			counterWidth = ( testWidth / guideline * 100 ) + '%';
			counterClass = 'tsf-pixel-counter-fit';
		}

		let sub = bar.firstChild;

		bar.classList.remove( fitClass, overflownClass )
		bar.classList.add( counterClass );
		sub.style.width = counterWidth;
	},
