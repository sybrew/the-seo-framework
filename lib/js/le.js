	...
	function _hijackListeners() {
		...
		if ( _oldInlineEditPost ) {
			window.inlineEditPost.edit = function( id ) {
				...
				// Initialize primary term selectors for list edit
				try {
					window.tsfPTLE?._initQuickEdit?.( id );
				} catch ( error ) {
					console.error( 'Error in tsfPTLE._initQuickEdit:', error );
				}
		...
		if ( _oldBulkEdit ) {
			window.inlineEditPost.setBulk = function() {
				...
				// Initialize primary term selectors for bulk edit
				try {
					window.tsfPTLE?._initBulkEdit?.();
				} catch ( error ) {
					console.error( 'Error in tsfPTLE._initBulkEdit:', error );
				}
