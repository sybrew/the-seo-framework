...
window.tsfLe = function () {
		...
		for ( const option in fieldsData ) {
			...
			if ( params.isSelect ) {
				...
				if ( option.startsWith( 'primary_term_' ) ) {
					...
					if ( params.options ) {
						for ( const [ value, label ] of Object.entries( params.options ) ) {
							if ( value != 0 ) { // Skip the default option as it's already added
	...
	/**
	 * Populates primary term dropdowns for bulk-edit.
	 *
	 * @since 5.1.3
	 * @access private
	 */
	function _populateBulkPrimaryTermDropdowns() {
		// For bulk-edit, we need to populate all hierarchical taxonomy terms
		// since we don't know which posts will be selected
		...
		selects.forEach( select => {
			...
			// For bulk-edit, we need to add common term options
			// This would ideally be populated from server-side data
			// For now, we ensure basic options are present
			const hasNoChange = select.querySelector( '[value="nochange"]' );
			const hasNone = select.querySelector( '[value="0"]' );
			...
			if ( ! hasNoChange ) {
				...
				select.insertBefore( noChangeOption, select.firstChild );
			}
			
			if ( ! hasNone ) {
				const noneOption = document.createElement( 'option' );
				noneOption.value = '0';
				noneOption.textContent = 'None (Clear primary term)';
