...
window.tsfLe = function () {
	...
	/**
	 * The term data obtained via tsfTermData.
	 * @typedef {?Object} termData
	 * @property {Object} primaryTerms
	 */
	let termData;
	...
	function _prepareTermInput( id ) {
		window.tsfPTLE._prepareQuickEditTerms( id, termData?.primaryTerms );
	}
	...
	function _hijackListeners() {
		...
		if ( _oldInlineEditPost ) {
			window.inlineEditPost.edit = function( id ) {
				...
				termData   = JSON.parse( document.getElementById( `tsfTermData[${id}]` )?.dataset.leTerm || 0 ) || {};
