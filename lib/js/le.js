...
window.tsfLe = function( $ ) {
	...
	const _setDefaultTitle = event => {
		let target          = ( event.originalEvent || event ).target,
			...
			oldDefaultTitle = tsfTitle.getStateFor( titleId, 'defaultTitle' ),
		...
		if ( defaultTitle !== oldDefaultTitle )
			tsfTitle.updateStateFor( titleId, 'defaultTitle', defaultTitle );
	}
	...
	const _prepareTitleInput = id => {
		...
		tsfTitle.updateStateFor( titleId, 'defaultTitle', data.defaultTitle.trim() );
		tsfTitle.updateStateFor( titleId, 'addAdditions', data.addAdditions );
		tsfTitle.updateStateFor( titleId, 'additionValue', data.additionValue.trim() );
	...
	const _prepareDescriptionInput = id => {
		...
		tsfDescription.updateStateFor( descId, 'defaultDescription', data.defaultDescription.trim() );
