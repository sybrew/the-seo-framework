...
window.tsfLe = function () {
	...
	function _setInlinePostValues() {
		for ( const option in fieldsData ) {
			...
			if ( params.isSelect ) {
				// Handle primary term dropdowns specially
				if ( option.startsWith( 'primary_term_' ) ) {
					const taxonomy = option.replace( 'primary_term_', '' );
					_populatePrimaryTermDropdown( element, taxonomy );
				}
	...
	/**
	 * Populates bulk-edit primary term dropdowns with available terms.
	 *
	 * @since 5.1.3
	 * @access private
	 */
	function _populateBulkPrimaryTermDropdowns() {
		// For bulk-edit, we need to populate with all available terms
		// We'll use WordPress' existing term data from the filter dropdowns
		const selects = document.querySelectorAll( '[id^="autodescription-bulk[primary_term_"]' );
		
		selects.forEach( select => {
			const taxonomyMatch = select.id.match( /primary_term_([^[\]]+)/ );
			if ( ! taxonomyMatch ) return;
			
			const taxonomy = taxonomyMatch[1];
			
			// Look for existing WordPress term filter dropdown
			const filterDropdown = document.getElementById( `${taxonomy}-filter-by` );
			if ( ! filterDropdown ) return;
			
			// Extract terms from the filter dropdown
			const filterOptions = filterDropdown.querySelectorAll( 'option' );
			...
			filterOptions.forEach( option => {
				...
				if ( value && value !== '0' && value !== 'all' && ! text.includes( 'All' ) ) {
	...
	/**
	 * Populates a primary term dropdown based on existing WordPress term data.
	 *
	 * @since 5.1.3
	 * @access private
	 ...
	 */
	function _populatePrimaryTermDropdown( element, taxonomy ) {
		// For quick-edit, get the post ID from the inline edit wrapper
		const postId = element.closest( '[id^="edit-"]' )?.id.replace( 'edit-', '' );
		if ( ! postId ) return;

		// Find the existing WordPress term data from the post row
		const postRow = document.getElementById( `post-${postId}` );
		if ( ! postRow ) return;

		// Look for existing taxonomy data in the row
		const taxonomyCell = postRow.querySelector( `.column-taxonomy-${taxonomy}` );
		if ( ! taxonomyCell ) return;

		// Extract term data from the existing WordPress display
		const termLinks = taxonomyCell.querySelectorAll( 'a' );
		const terms = [];
		
		termLinks.forEach( link => {
			const href = link.getAttribute( 'href' );
			const termIdMatch = href?.match( /tag_ID=(\d+)/ );
			if ( termIdMatch ) {
				terms.push( {
					id: termIdMatch[1],
	...
	return {
		...
		load: () => {
			...
			// Also initialize bulk-edit dropdowns when the page loads
			document.body.addEventListener( 'tsf-onload', _populateBulkPrimaryTermDropdowns );
		},
	};
}();
window.tsfLe.load();
