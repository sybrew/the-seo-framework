		...
		if ( canonicalInput && inlineEditWrap ) switch ( _editType ) {
			case 'post':
				...
				const updateCanonical = tsfUtils.debounce(
					() => {
						let slug = postSlugInput?.value || postTitleInput?.value;

						slug = tsfCanonical.sanitizeSlug( slug.substring( 0, 200 ) )
						...
						urlDataParts
							...
							.set( `%post_id%`, id )
							.set( `%postname%`, slug ) // We rewrote %pagename% to %postname% at `Meta\URI\Utils::get_url_permastruct()`.
							.set( `%category%`, '' ) // var_dump() TODO
							.set( `%post_tag%`, '' ) // var_dump() TODO
							.set( `%author%`, '' ); // var_dump() TODO
			...
			case 'tax':
				...
				if ( taxonomy ) {
					...
					const termUpdateCanonical = tsfUtils.debounce(
						() => {
							let slug = termSlugInput?.value || termNameInput?.value;
