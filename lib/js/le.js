...
window.tsfLe = function() {
	...
	const _dispatchUpdate = tsfUtils.debounce(
		() => document.body.dispatchEvent( new CustomEvent( 'tsfLeUpdated' ) ),
		50, // Magic number. Low enough not to visual glitches, high enough not to cause lag.
	);
	 ...
	 * @param {Event} event
	 */
	function _setTitleVisibilityPrefix( event ) {
		...
		if ( 'keep_private' === target.name ) {
			...
			visibility = target.value?.length ? 'password' : 'public';
		}
	 ...
	 * @param {Event} event
	 */
	function _setDefaultTitle( event ) {
		const target     = ( event.originalEvent || event ).target,
			  titleId    = target.dataset?.tsfTitleId,
