...
window.tsfSettings = function( $ ) {
	...
	const _initPtaSelector = () => {
		...
		// TODO I think it best to simply show "post type is disabled." Please reenable it to augment settings.

			const option = select.querySelector( `option[value=${postType}]` );
			if ( ! option ) return;

			if ( set.has( postType ) ) {
				removedOptions.set( postType, option.cloneNode( true ) );
				option.disabled = true;
				option.remove();

				// Forward first non-disabled option (if any), and select that.
				tsf.selectByValue( select, select.querySelector( 'option:not([disabled])' )?.value || '' );
				select.dispatchEvent( new Event( 'change' ) );
			} else {
				option.disabled = false;

				const _option = removedOptions.get( postType );
				if ( _option ) {
					// TODO set right level... -> reuse initial "populateSelect" loop?
					select?.appendChild( _option );
				}
			}
		}
