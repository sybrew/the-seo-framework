...
window.tsfSettings = function( $ ) {
	...
	const _initGeneralSettings = () => {
		 ...
		 * @return {string} The cloned input class used for sending POST data.
		 */
		const getCloneClassTax = taxonomy => tsf.escapeString( 'tsf-disabled-taxonomy-input-clone-' + taxonomy );
		/**
		 * @param {string} postType
		...
		const validateTaxonomies = ( event, taxonomy ) => {
			...
			if ( getValidateTaxonomiesCache( 'disabledTaxonomiesAll' ).size !== disabledTaxonomiesAll.size ) {
				// disabledTaxonomiesAll.has( taxonomy )
			}
	...
	const _initTabs = () => {
		...
		const tabToggle = ( event, onload ) => {
			...
			if ( onload ) {
				...
				toggleTimeouts[ toggleName ].hasOwnProperty( 'addClass' ) && clearTimeout( toggleTimeouts[ toggleName ].addClass );
				toggleTimeouts[ toggleName ].hasOwnProperty( 'trigger' ) && clearTimeout( toggleTimeouts[ toggleName ].trigger );
