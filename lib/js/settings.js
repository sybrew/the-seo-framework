...
window.tsfSettings = function( $ ) {
	...
	const _initGeneralSettings = () => {
		...
		const validateTaxonomies = event => {
			...
			if ( getValidateTaxonomiesCache( 'excludedPtTaxonomies' ).size !== setPt.size ) // Redundant????
			...
			validateTaxonomiesCache.set( 'excludedTaxonomiesAll', new Set( setAll ) );
			validateTaxonomiesCache.set( 'excludedTaxonomies', new Set( set ) );
			validateTaxonomiesCache.set( 'excludedPtTaxonomies', new Set( setPt ) );
		}
	...
	const _initTitleSettings = () => {
		...
		const toggleAdditionsLocationExample = event => {
			...
			const showLeft      = 'left' === event.target.value,
		 ...
		 */
		 const adjustSiteTitleExampleOuput = event => {
			...
			if ( oldVal !== newVal ) {
				...
				tsfTitle.updateStateAll( 'additionValue', homeTitleId );
	...
	const _initPtaSettings = () => {
		...
		if ( Object.keys( postTypeData ).length > 1 )
			_initPtaSelector();
	...
	const _initPtaSelector = () => {
		...
		const switchPostTypeSettingsView = event => {
			...
			_debounceSwitch = setTimeout( () => {
				...
				_detailsEl && document.dispatchEvent(
	 ...
	 */
	 const _initPtaTitleSettings = postType => {
		...
		_inputEl.addEventListener( 'input', event => {
			let showPrefix = ! event.target.value.length;
		...
		const updateTitleRemoveAdditions = event => {
			...
			if ( removeAdditions ) {
				blogNameTrigger.insertAdjacentHTML( 'beforeend', disabledTitleAdditionsHelp );
				tsfTT.triggerReset();
			} else {
				// 'tsf-title-additions-warning' is defined at `../inc/views/templates/settings/settings.php`
				blogNameTrigger.querySelector( '.tsf-title-additions-warning' )?.remove();
			}
	...
	const _initPtaVisibilitySettings = postType => {
		...
		const setDefaultRobotsValue = robotsType => {
			const indexSelect = document.getElementById( _getPtaInputId( postType, robotsType ) );

			const _defaultIndexOption = indexSelect?.querySelector( '[value="0"]' ),
				  _data               = indexSelect?.dataset || {};

			let newHTML = _data.defaultI18n?.replace(
				'%s',
				tsf.decodeEntities(
					robotsData.site.get( robotsType ) || robotsData.pt.get( robotsType )
						? _data.defaultOff
						: _data.defaultOn
				)
			);
	...
	const _initPtaMainListeners = postType => {
		...
		document.addEventListener( 'tsf-post-type-archive-switched', triggerPtaSynchronousUnregisteredInput );
		...
		const augmentPTRobots = event => {
			...
			if ( set.has( postType ) ) {
				getPostTypeRobotsSettings( postType ).forEach( element => {
					...
					element.closest( 'label' ).insertAdjacentHTML( 'beforeend', postTypeHelpTemplate );
					element.closest( 'label' ).append( clone );
				} );
		...
		const augmentTaxonomyRobots = event => {
			...
			if ( setAll.has( taxonomy ) ) {
				getTaxonomyRobotsSettings( taxonomy ).forEach( element => {
					...
					if ( ! element.closest( 'label' ).querySelector( '.tsf-taxonomy-warning' ) )
						element.closest( 'label' ).insertAdjacentHTML( 'beforeend', taxonomyHelpTemplate );

					if ( ! element.closest( 'label' ).querySelector( getCloneClassTaxonomy( taxonomy ) ) )
						element.closest( 'label' ).append( clone );
				} );
	 ...
	 */
	const _disPatchAtInteractive = ( element, eventName ) => {
	...
	return Object.assign( {
		...
		load: () => {
			...
			).forEach( el => { el.insertAdjacentElement( 'afterend', headerEnd ) } );
