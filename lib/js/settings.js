...
window.tsfSettings = function( $ ) {
		...
		const tabToggle = ( event, onload ) => {
			...
			if ( onload ) {
				...
				if ( 'number' !== typeof toggleTimeouts[ toggleName ].fadeOut ) {
					...
					$allContent.stop( false, true ).fadeOut( fadeOutTimeout, function() {
				...
				toggleTimeouts[ toggleName ].addClass = setTimeout( () => {
					...
					$newContent.stop( false, true ).addClass( activeClass ).fadeIn( 250 );
				}, fadeOutTimeout );
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			// Execute this ASAP, to prevent bouncing.
