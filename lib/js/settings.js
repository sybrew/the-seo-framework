...
window.tsfSettings = function( $ ) {
		...
		const toggleAdditionsDisplayExample = () => {
			...
			if ( $( additionsToggle ).is( ':checked' ) ) {
				...
				if ( socialAdditionsToggle ) {
					socialAdditionsToggle.dataset.hasWarning = true;
	...
	const _initHomeTitleSettings = () => {
		...
		titleInput.placeholder = tsf.escapeString( tsf.decodeEntities( state.placeholder.trim() ) );

		tsfTitle.updateStateFor( titleId, 'allowReferenceChange', ! state.refTitleLocked );
		tsfTitle.updateStateFor( titleId, 'defaultTitle', tsf.escapeString( tsf.decodeEntities( state.defaultTitle.trim() ) ) );
		tsfTitle.updateStateFor( titleId, 'useTagline', state.useTagline );
		tsfTitle.updateStateFor( titleId, 'useSocialTagline', !! ( state.useSocialTagline || false ) );
		tsfTitle.updateStateFor( titleId, 'additionValue', tsf.escapeString( tsf.decodeEntities( state.additionValue.trim() ) ) );
		tsfTitle.updateStateFor( titleId, 'additionPlacement', state.additionPlacement );
		tsfTitle.updateStateFor( titleId, 'hasLegacy', !! ( state.hasLegacy || false ) );
		...
		const setHoverAdditionsPlacement = () => {
			let oldPlacement = tsfTitle.getStateFor( titleId, 'additionPlacement' ),
				placement    = 'after';
			...
			if ( placement !== oldPlacement ) {
				tsfTitle.updateStateFor( titleId, 'additionPlacement', placement );
			}
		}
		...
		const setTitleVisibilityPrefix = ( visibility ) => {
			...
			let oldPrefixValue = tsfTitle.getStateFor( titleId, 'prefixValue' ),
				prefixValue    = '';
		...
		const adjustHomepageExampleOutput = ( event ) => {
			...
			if ( val.length ) {
				...
				$example.html( tsf.escapeString( tsf.decodeEntities( tsfTitle.getStateFor( titleId, 'defaultTitle' ) ) ) );
			}
		};
		$( titleInput )
		...
		const updateHomePageTaglineExampleOutput = () => {
			...
			updateHomePageTaglineExampleOutputBuffer = setTimeout( () => {
				let value = tsfTitle.getStateFor( titleId, 'additionValue' );

				value = tsf.decodeEntities( tsf.sDoubleSpace( value.trim() ) );

				if ( value.length && tsfTitle.getStateFor( titleId, 'useTagline' ) ) {
		...
		const updateHoverAdditionsValue = () => {
			let oldValue = tsfTitle.getStateFor( titleId, 'additionValue' ),
			...
			if ( oldValue !== value ) {
				tsfTitle.updateStateFor( titleId, 'additionValue', value );
		...
		const toggleHomePageTaglineExampleDisplay = ( event ) => {
			...
			tsfTitle.updateStateFor( titleId, 'useTagline', useTagline );
			updateHomePageTaglineExampleOutput();
		}
		$( taglineToggle )
	...
	const _initHomeDescriptionSettings = () => {
		...
		tsfDescription.updateStateFor( descId, 'defaultDescription', tsf.escapeString( state.defaultDescription.trim() ) );
		tsfDescription.updateStateFor( descId, 'hasLegacy', !! ( state.hasLegacy || false ) );
	...
	const _initTabs = () => {
		...
		const tabToggle = ( event, onload ) => {
			...
			// Why is this here? For init trigger?
			...
			if ( onload ) {
				document.querySelectorAll( '.' + toggleName + '-content' ).forEach( element => {
					element.classList.remove( activeClass );
					element.style.display = 'none';
				} );
				let newContent = document.getElementById( toggleId + '-content' );
				newContent.classList.add( activeClass );
				newContent.style.display = null;

				$( '#' + toggleId ).trigger( 'tsf-tab-toggled' );
			} else {
				if ( ! toggleTimeouts.hasOwnProperty( toggleName ) ) {
					toggleTimeouts[ toggleName ] = {
						fadeOut:  void 0,
						addClass: void 0,
						trigger:  void 0,
					};
				...
				clearTimeout( toggleTimeouts[ toggleName ].addClass );
				clearTimeout( toggleTimeouts[ toggleName ].trigger );

				if ( 'number' !== typeof toggleTimeouts[ toggleName ].fadeOut ) {
					// Set arbitrary timeout so the fadeout won't run twice.
					// Always complete the fadeout, even during debouncing.
					toggleTimeouts[ toggleName ].fadeOut = setTimeout( () => {
						toggleTimeouts[ toggleName ].fadeOut = undefined;
					}, fadeOutTimeout );

					let $allContent = $( '.' + toggleName + '-content' );
					$allContent.fadeOut( fadeOutTimeout, function() {
						$( this ).removeClass( activeClass );
					} );
				}

				toggleTimeouts[ toggleName ].addClass = setTimeout( () => {
					let $newContent = $( '#' + toggleId + '-content' );
					$newContent.addClass( activeClass ).fadeIn( 250 );
				}, fadeOutTimeout );
				toggleTimeouts[ toggleName ].trigger = setTimeout( () => {
					$( '#' + toggleId ).trigger( 'tsf-tab-toggled' );
				}, fadeOutTimeout + 25 );
			}
		}
