...
window.tsfSettings = function( $ ) {
	...
	function _initSitemapInputs() {
		...
		const optimizedSitemapsToggle = document.getElementById( _getSettingsId( 'sitemaps_output' ) ),
			  ...
			  yearlySitemapsToggle    = document.getElementById( _getSettingsId( 'sitemaps_yearly' ) ),
			  monthlySitemapsToggle   = document.getElementById( _getSettingsId( 'sitemaps_monthly' ) );
		/**
		 * @function
		 * @param {Event} event
		 */
		const updateSocialAdditions = event => {
			...
			// Hide yearly sitemap settings when optimized sitemap is disabled
			document.getElementById( 'tsf-yearly-sitemap-settings' )
				?.classList.toggle( 'hidden', ! sitemapsEnabled );
		}
		...
		/**
		 * Toggle monthly sitemaps based on yearly sitemaps setting
		 * @function
		 * @param {Event} event
		 */
		const toggleMonthlySitemaps = event => {
			const yearlyEnabled = !! event.target.checked;

			document.getElementById( 'tsf-monthly-sitemap-settings' )
				?.classList.toggle( 'hidden', ! yearlyEnabled );
		}
		if ( yearlySitemapsToggle ) {
			yearlySitemapsToggle.addEventListener( 'change', toggleMonthlySitemaps );
			_dispatchAtInteractive( yearlySitemapsToggle, 'change' );
		}
