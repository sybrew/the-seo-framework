...
window.tsfSettings = function( $ ) {
	...
	const _initGeneralSettings = () => {
		...
		const validatePostTypes = event => {
			augmentPTRobots( event.detail.postType, excludedPostTypes.has( event.detail.postType ) );
		...
		const validateTaxonomies = event => {
			...
			// TODO move this to the robots-settings handler.
		...
		document.body.addEventListener( 'tsf-post-type-support-changed', validatePostTypes );
	 ...
	 */
	 const _initPtaTitleSettings = postType => {
		...
		if ( blogNameTrigger ) {
			const triggerBlogNameChange = () => blogNameTrigger.dispatchEvent( new Event( 'change' ) );

			blogNameTrigger.addEventListener( 'change', updateTitleAdditions );
			triggerBlogNameChange();

			document.body.addEventListener( 'tsf-update-title-rem-additions', event => {
	...
	const _initRobotsInputs = () => {
		...
		document.querySelectorAll( '.tsf-robots-site' ).forEach( el => {
			el.addEventListener( 'change', dispatchSiteRobotsChangedEvent );
			el.dispatchEvent( changeEvent );
		} );
