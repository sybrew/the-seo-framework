...
window.tsfSettings = function( $ ) {
	...
	const _initGeneralSettings = () => {
		...
		document.querySelectorAll( '.tsf-disabled-post-types' ).forEach( el => {
			el.addEventListener( 'change', checkDisabledPT );
			el.dispatchEvent( changeEvent );
		} );
		document.querySelectorAll( '.tsf-disabled-taxonomies' ).forEach( el => {
	...
	const _initHomeGeneralListeners = () => {
		...
		let homepageGeneralTab = document.getElementById( 'tsf-homepage-tab-general' );
	...
	const _initTabs = () => {
		...
		const tabToggle = ( event ) => {
			...
			const activeClass       = 'tsf-tab-content-active',
