...
window.tsfSettings = function( $ ) {
	...
	const _initHomeTitleSettings = () => {
		...
		let titleInput    = document.getElementById( 'autodescription-site-settings[homepage_title]' ),
		...
		const setTitleVisibilityPrefix = ( visibility ) => {
			...
			switch ( visibility ) {
				case 'password':
					prefixValue = l10n.i18n.protectedTitle;
					break;

				case 'private':
					prefixValue = l10n.i18n.privateTitle;
					break;
		...
		const adjustHomepageExampleOutput = ( event ) => {
			...
			if ( val.length ) {
				...
				$example.text( tsfTitle.getState( 'defaultTitle' ) );
			}
		};
		...
		const updateHomePageTaglineExampleOutput = () => {
			...
			updateHomePageTaglineExampleOutputBuffer = setTimeout( () => {
				...
				if ( value.length && tsfTitle.getState( 'useTagline' ) ) {
					$exampleTagline.text( value );
					// $exampleTagline.text( tsf.escapeString( tsf.sDoubleSpace( value ) ) );
		...
		const updateHoverAdditionsValue = () => {
			let oldValue = tsfTitle.getState( 'additionValue' ),
				value    = tsf.sDoubleSpace( taglineInput.value.trim() );
