...
window.tsfTerm = function( $ ) {
	...
	const _initTitleListeners = () => {
		...
		let metaInput = document.getElementById( 'autodescription-meta[doctitle]' );

		tsfTitle.setInputElement( metaInput );

		//!? Disabled as we don't yet listen to a blogname option.
		// Also, this is the code for the homepage, not terms...
		//
		// /**
		...
		//  * @return {undefined}
		//  */
		// const updateTitleAdditions = function( event ) {
		...
		const updateDefaultTitle = function( val ) {
			...
			if ( val.length ) {
				name = tsf.escapeString( l10n.params.stripTitleTags ? tsf.stripTags( val ) : val );
			} else {
				name = tsfTitle.l10n.params.untitledTitle;
			}

			if ( tsf.l10n.states.isRTL ) {
				defaultTitle = name + ' ' + l10n.params.termPrefix;
			} else {
				defaultTitle = l10n.params.termPrefix + ' ' + name;
			}
	...
	const _readySettings = () => {
		...
		tsfSocial.initTitleInputs( {
			meta: document.getElementById( 'tsf-title-reference' ),
		...
		tsfSocial.initDescriptionInputs( {
			meta: document.getElementById( 'tsf-description-reference' ),
