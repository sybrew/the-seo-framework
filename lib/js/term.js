...
window.tsfTerm = function() {
	...
	function _initVisibilityListeners() {
		...
		const taxonomy = document.querySelector( '[name=taxonomy]' )?.value || 'category';
		...
		if ( canonicalInput ) {
			...
			const updateCanonical = () => {
				...
				urlDataParts.set(
					`%${taxonomy}%`,
					slugInput.value.length ? slugInput.value : nameInput.value,
				);
				updateCanonicalPlaceholder();
			}

			slugInput.addEventListener( 'change', updateCanonical );
			nameInput.addEventListener( 'change', updateCanonical );
