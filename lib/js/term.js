...
window.tsfTerm = function () {
	...
	function _initVisibilityListeners() {
		...
		if ( state ) {
			...
			// We don't set parentTermSlugs here. They aren't something tsfCanonical can work with.
		}
		...
		if ( canonicalInput ) {
			...
			let parentTermSlugs = state.parentTermSlugs;
			...
			parentSelect?.addEventListener( 'change', updateParentTermSlugs );
