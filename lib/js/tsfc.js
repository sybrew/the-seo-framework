...
window.tsfC = function( $ ) {
	 ...
	 */
	let l10n = 'undefined' !== typeof tsfCL10n && tsfCL10n;
	 ...
	 * @return {undefined}
	 */
	const _toggleCharacterCounterDisplay = ( event ) => {
		if ( $( event.target ).is( ':checked' ) ) {
			$( '.tsf-counter-wrap' ).show();
		} else {
			$( '.tsf-counter-wrap' ).hide();
		}
	}
	 ...
	 * @return {undefined}
	 */
	const _togglePixelCounterDisplay = ( event ) => {
		if ( $( event.target ).is( ':checked' ) ) {
	...
	const _initCounters = () => {
		...
		// SEO Settings page
		$( '#autodescription-site-settings\\[display_character_counter\\]' ).on( 'click', _toggleCharacterCounterDisplay );
		$( '#autodescription-site-settings\\[display_pixel_counter\\]' ).on( 'click', _togglePixelCounterDisplay );
	}
