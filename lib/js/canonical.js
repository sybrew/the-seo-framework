...
window.tsfCanonical = function () {
	 ...
	 *
	 * This function is based on WP Core sanitize_title_with_dashes(), which is used to sanitize slugs.
	 *
	 * @since 5.0.7
	 * @access public
	 *
	...
	function sanitizeSlug( slug ) {
		...
		if ( 'string' !== typeof slug ) return slug;
		...
		slug = slug.replace( /<\/?[^>]+(>|$)/g, '' )             // Strip HTML tags
			...
			.replace( /%/g, '')                                  // Remove percent signs not part of an octet
			.replace( /---([a-fA-F0-9][a-fA-F0-9])---/g, '%$1' ) // Restore octets
			.replace( /\s+/g, '-' )                              // Convert spaces to hyphens
			.toLowerCase();                                      // Convert to lowercase and encode UTF-8 characters
	...
	function _updatePlaceholder( event ) {
		...
		if ( getStateOf( id, 'showUrlPlaceholder' ) ) {
			if ( getStateOf( id, 'allowReferenceChange' ) ) {
				const urlStructure    = getStateOf( id, 'urlStructure' ),
				...
				rewrite.code.forEach( ( code, index ) => {
					...
					if ( code in urlDataParts ) {
						// WordPress's rewrite rules account for edge-cases, vestigal code, and other oddities.
						...
						// So, we disable this for now and assume the given content is good for URLs. URL.parse() fixes most issues.
