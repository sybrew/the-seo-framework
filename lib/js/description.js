...
window.tsfDescription = function( $ ) {
	 ...
	 */
	const getState = part => {
		tsf.deprecatedFunc( 'tsfDescription.getState()', '4.1.0', 'tsfDescription.getStateFor()' );
	...
	const updateState = ( part, value ) => {
		tsf.deprecatedFunc( 'tsfDescription.updateState()', '4.1.0', 'tsfDescription.updateStateFor()' );
	...
	const _setReferenceDescription = event => {
		let references           = _getDescriptionReferences( event.target.id ),
			...
			text                 = allowReferenceChange && event.target.value || getStateFor( event.target.id, 'defaultDescription' ) || '';
