...
window.tsfGBC = function( $ ) {
	...
	function GBCTooltip( { element, desc, setTooltip, tt } ) {
		...
		const isImage = desc.startsWith( '<img' );
		const className = [
			ttNames.base,
			isImage && 'tsf-is-tt-type-image'
		].filter( Boolean ).join(' ');
		...
		return createElement( wp.components.Popover,
			{
				ref: ( popover ) => {
					...
					setTooltip( popover );
					// Wait a frame so the bounding rectangle of the Popover component is
					// available. For adjacent tooltips, the rectangle is available yet it
					// can be stale if its width is different because the popover has not
					// yet repositioned.
				...
				animate: false,
				shift: true,
				// Enable resizing when it’s an image tooltip. Gutenberg’s docs say `resize` should
				// not be enabled at the same time as `shift` yet it seems to work well enough in
				// most cases. In rare cases however the arrow can horizontally detach from the
				// resized popover. Yet, if `shift` is disabled the the tooltip can be placed
				// partially offscreen horizontally. Barring some upstream changes that expose more
				// flexible settings, the detachment issue might be solvable by adding a resize
				// observer and shifting the tooltip horizontally as it resizes. Alternatively,
				// disabling `shift` and contextually applying horizontally aligned placements like
				// 'top-start' or 'top-end' might be workable.
				resize: isImage,
				variant: 'unstyled',
				placement: 'top',
				inline: true,
