...
// @TODO Set up externs in GitHub
// @js_externs /** @constructor */ function autodescription() {}; /** @function */ autodescription.statusBarHover; /** @type {Array|string} */ var autodescriptionL10n; /** @constructor */ function doCrop() {}; /** @type {(Object<string, *>|Array<Object<string, *>>)} * @const */ wp.media.controller; /** @type {!Object<string, string>} */ wp.media.controller.prototype.Cropper = {}; /** @type {!Object<string, string>} */ wp.media.controller.prototype.Library = {}; /** @constructor */ function setState() {};
// ==/ClosureCompiler==
...
window[ 'autodescription' ] = {
	 ...
	 */
	hasInput : autodescriptionL10n['hasInput'],
	...
	updateCharacterCountDescription: function( event ) {
		...
		var $this = jQuery( event.target ),
			...
			$phLength = $this.prop( 'placeholder' ).length,
	...
	openImageEditor: function( event ) {
		...
		var $target = jQuery( event.target ),
			inputIDURL = $target.data( 'inputid_url' ),
			inputIDID = $target.data( 'inputid_id' ),
			frame;
		...
		//* Init extend cropper.
		autodescription.extendCropper();

		frame = wp.media( {
			button : {
				text : autodescriptionL10n['useThisImage'],
				close : false,
			},
			states: [
				new wp.media.controller.Library( {
					title: autodescriptionL10n['selectSocialImage'],
				...
				new autodescription.cropper( {
					imgSelectOptions: autodescription.calculateImageSelectOptions
				} ),
			],
		} );
		...
		frame.on( 'cropped', function( croppedImage ) {
			...
			jQuery( '#' + inputIDURL ).val( url );
			jQuery( '#' + inputIDID ).val( attachmentId );
		} );
	...
	extendCropper: function() {
		...
		if ( 'undefined' !== typeof autodescription.cropper.control )
			return;

		/**
		 * autodescription.extendCropper => wp.media.controller.AutodescriptionCropper
		 *
		...
		var Controller = wp.media.controller,
			AutodescriptionCropper;

		AutodescriptionCropper = Controller.Cropper.extend( {
			doCrop: function( attachment ) {
				var cropDetails = attachment.get( 'cropDetails' ),
					control = autodescription.cropper.control,
				...
				if ( control.params.flex_width && control.params.flex_height ) {
					...
					// Landscape/Portrait
						...
						// Constrain flexible sides based on the image ratio.
						}
					}
				}
				...
				/**
				 * TODO FIX THIS.
				 * Closure compiler destroys this part.
				 */
		...
		AutodescriptionCropper.prototype.control = {};
		AutodescriptionCropper.control = {
			params : {
				flex_width : 1500,
				flex_height : 1500,
				width : 1200,
		...
		autodescription.cropper = AutodescriptionCropper;
	...
	calculateImageSelectOptions: function( attachment, controller ) {
		...
		var control = autodescription.cropper.control,
		...
		controller.set( 'canSkipCrop', ! autodescription.mustBeCropped( control.params.flex_width, control.params.flex_height, realWidth, realHeight ) );
	...
	ready: function( jQ ) {
		...
		jQ( '.tsf-js-confirm-reset' ).on( 'click.autodescription.autodescription_confirm_reset', autodescription.confirmedReset );
		...
		// Image uploader
		jQ( '.tsf-set-social-image' ).on( 'click', autodescription.openImageEditor );
	}
};
