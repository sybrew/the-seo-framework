...
/* global settingsChanged, autodescription, autodescriptionL10n, titleTagline, titleAdditions, blogDescription, siteTitle, titleSeparator, saveAlert, confirmReset */
...
window[ 'autodescription' ] = {
	...
	dynamicPlaceholder: function( event ) {
		...
		var $this = jQuery( event.target ),
			...
			$leftPad = ( $this.outerWidth() - $this.width() ) / 2,
		...
		if ( $this.outerWidth() > $leftOffset )
			$placeholder.css( 'left', $leftPad + $leftOffset + $offsetTest.width() + "px" );
	...
	selectTitleInput: function() {
		...
		var $input = jQuery( "#autodescription_title, #autodescription-meta\\[doctitle\\]" );
	...
	ready: function() {
		...
		jQuery( "#autodescription_title, #autodescription-meta\\[doctitle\\]" ).on( 'keydown keyup', autodescription.dynamicPlaceholder );
