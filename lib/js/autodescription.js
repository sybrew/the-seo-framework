...
window[ 'autodescription' ] = {
	...
	setAjaxLoader: function( target ) {
		...
		jQuery( target ).toggleClass( 'theseoframework-loading' );
	},
	...
	unsetAjaxLoader: function( target, success ) {
		...
		var $newclass = 'theseoframework-success',
			$fade = 2500;

		if ( ! success ) {
			$newclass = 'theseoframework-error';
			$fade = 5000;
		}

		jQuery( target ).removeClass( 'theseoframework-loading' ).addClass( $newclass ).fadeOut( $fade );
	},
	...
	resetAjaxLoader: function( target ) {
		...
		jQuery( target ).stop().empty().attr( 'class', 'theseoframework-ajax' ).css( 'opacity', '1' ).removeAttr( 'style' );
	},
	...
	counterUpdate: function( event ) {
		...
		var $target = '.theseoframework-counter .theseoframework-ajax',
			$status = 0;
