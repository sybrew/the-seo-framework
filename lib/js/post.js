...
window.tsfPost = function () {
	 ...
	 * @since 5.0.7 Refactored to support dynamic URL structures.
	 * @access private
	 */
	function _initVisibilityListeners() {
		...
		if ( canonicalInput ) {
			let pageSlugs  = {}; // This needs to be collapsed
			let termSlugs  = {}; // This needs to be collapsed
			let author     = '';
			let dateString = '';
			let title      = '';
			let slug       = '';

			const writeSlug   = tsfCanonical.structIncludes( _canonicalId, '%postname%' ); // ignore '%pagename%', it's broken.
			...
			for ( const [ taxonomy, terms ] of Object.entries( state.parentTermSlugs ) ) {
				...
				termSlugs[ taxonomy ] = terms;
				writeTerm[ taxonomy ] = tsfCanonical.structIncludes( _canonicalId, `%${taxonomy}%` ); // Should always be true...
			}

			// Unpack author slugs.
			if ( writeAuthor ) {
				Object.entries( state.authorSlugs ).forEach( ( [ authorId, authorSlug ] ) => {
					tsfAuthorSlugs.store( authorId, authorSlug );
					author = authorSlug; // There should only be one.
				} );
			}
			 ...
			 *
			 * There are unlimited possibilities for the URL structure, but, for now, we'll only support
			 * the most common ones, because the entire thing is a mess in Core.
			 *
			 ...
			 *
			 * %post_tag% ought to be valid, but it's vestigal and does nothing.
			 *
			 * @function
			 */
			const updateCanonical = () => {
				// Slug falls back to the title, also if 0. However, if the title is '0', it'll be used.
				// WordPress fills in the slug for us, handling these edge-cases. Hence, we don't test the slug.length.
				const activeSlug = slug || title || '';

				// postName always gets trimmed to the first 200 characters.
				const postName = encodeURIComponent(
					activeSlug.substring( 0, 200 ).toLowerCase().replace( /\s+/g, '-' )
				);

				// Just write these without checks; there's no meaningful performance hit.
				urlDataParts
					.set( `%post_id%`, l10n.states.id )
					.set( `%postname%`, postName ) // Unlike %pagename%, this is for all post types.
					.set( `%author%`, author );
			...
			document.addEventListener(
				...
				async event => {
					...
					switch ( event.detail.type ) {
						...
						case 'author':
							if ( writeAuthor ) {
								author = await tsfAuthorSlugs.get( event.detail.postData.get( 'author' ) );
								queueUpdateCanonical();
							}
							break;
						case 'date':
			...
			if ( ! l10n.states.isGutenbergPage ) {
				if ( writeSlug ) {
					...
					const updatePostName = () => {
						...
						slug  = postNameInput?.value ?? document.getElementById( 'editable-post-name-full' )?.innerText;
						queueUpdateCanonical();
					}
				...
				if ( writeAuthor ) {
					...
					if ( authorIdInput ) {
						...
						const updateAuthor = tsfUtils.debounce(
							async () => {
								author = await tsfAuthorSlugs.get( authorIdInput.value );
								queueUpdateCanonical();
							},
							100, // Magic number. High enough to prevent self-DoS, low enough to be responsive.
						);
				...
				if ( writeDate ) {
					const dateFields = [
						// These fields don't show the date very accurately when posted "immediately" (new post), TODO fixme? We'd need to run a clock.
					...
					const updateDateString = () => {
						dateString = new Date( ...getRealDateValues() ).toISOString();
						queueUpdateCanonical();
					}
