...
window.tsfPost = function( $ ) {
	...
	const _initTitleListeners = () => {
		...
		tsfTitle.updateStateFor( _titleId, 'useTagline', state.useTagline );
		...
		const updateTitleAdditions = ( event ) => {
			let prevUseTagline = tsfTitle.getStateFor( _titleId, 'useTagline' ),
			...
			if ( prevUseTagline !== useTagline ) {
				tsfTitle.updateStateFor( _titleId, 'useTagline', useTagline );
			}
		}
		$( blogNameTrigger )
