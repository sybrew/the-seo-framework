...
window.tsfPost = function() {
	...
	function _doFlexResizeListener() {
		...
		const calculateTextOverflow = target => {
			...
			if ( displayNames ) {
				...
				navNames.forEach( element => { tsfUI.fadeIn( element ) } );
			} else {
				...
				navNames.forEach( element => { tsfUI.fadeOut( element, 0 ) } );
			}
	...
	function _initVisibilityListeners() {
		...
		document.addEventListener(
			'tsf-updated-block-editor-link',
			event => updateCanonical( event.detail.value ),
		);
