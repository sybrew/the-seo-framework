	...
	function _initVisibilityListeners() {
		...
		if ( canonicalInput ) {
			...
			const updateCanonical = data => {
				...
				const activeSlug = data.slug || data.title || '';
			...
			document.addEventListener(
				...
				event => {
					...
					switch ( event.detail.type ) {
						case 'link': // Not used, but indicates something changed.
						case 'title':
						case 'slug': // redundant?
						case 'author':
						case 'date':
			...
			if ( ! l10n.states.isGutenbergPage ) {
				...
				const classicUpdateCanonical = tsfUtils.debounce(
					() => {
						updateCanonical( {
							...
							author: authorId?.value, // var_dump() this must be nicename.
							date:   new Date( ...getRealDateValues() ).toISOString(),
						} );
					},
					1000/60, // 60fps
				);
				...
				postName?.addEventListener( 'input', classicUpdateCanonical );
				authorId?.addEventListener( 'input', classicUpdateCanonical );

				dateFields.forEach( field => {
					field?.addEventListener( 'change', classicUpdateCanonical );
				} );
