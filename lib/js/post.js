...
window.tsfPost = function( $ ) {
	...
	const _initUpdateMetaBox = () => {
		...
		let seobar           = document.querySelector( '.tsf-seo-bar' ),
			// We only do this because it looks nice. The rest is implied via the counter updates.
			seobarAjaxLoader = document.querySelector( '#tsf-doing-it-right-wrap .tsf-ajax' );

		let desc   = document.getElementById( 'autodescription_description' ),
		...
		let imageUrl = document.getElementById( 'autodescription_socialimage-url' );
			...
			switch ( response.type ) {
				case 'success':
					...
					setTimeout( () => {
						...
						// Assume that the user didn't have the time to adjust the fields. This is better than fixing a race condition.
						tsfAys && tsfAys.reset();
					}, fadeTime );
		...
		$( window ).on( 'tsf-gutenberg-saved-document', () => {
