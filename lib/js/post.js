...
window.tsfPost = function( $ ) {
	...
	const _initTitleListeners = () => {
		...
		tsfTitle.updateStateFor( _titleId, 'allowReferenceChange', ! state.refTitleLocked );
		tsfTitle.updateStateFor( _titleId, 'defaultTitle', tsf.escapeString( tsf.decodeEntities( state.defaultTitle.trim() ) ) );
		tsfTitle.updateStateFor( _titleId, 'addAdditions', state.addAdditions );
		tsfTitle.updateStateFor( _titleId, 'useSocialTagline', !! ( state.useSocialTagline || false ) );
		tsfTitle.updateStateFor( _titleId, 'additionValue', tsf.escapeString( tsf.decodeEntities( state.additionValue.trim() ) ) );
		tsfTitle.updateStateFor( _titleId, 'additionPlacement', state.additionPlacement );
		tsfTitle.updateStateFor( _titleId, 'hasLegacy', !! ( state.hasLegacy || false ) );
		...
		const updateTitleAdditions = ( event ) => {
			let prevAddAdditions = tsfTitle.getStateFor( _titleId, 'addAdditions' ),
			...
			if ( prevAddAdditions !== addAdditions ) {
				tsfTitle.updateStateFor( _titleId, 'addAdditions', addAdditions );
			}
		}
		$( blogNameTrigger )
		...
		const setTitleVisibilityPrefix = ( visibility ) => {
			...
			let oldPrefixValue = tsfTitle.getStateFor( _titleId, 'prefixValue' ),
				prefixValue    = '';
			...
			if ( prefixValue !== oldPrefixValue )
				tsfTitle.updateStateFor( _titleId, 'prefixValue', prefixValue );
		}
		...
		const updateDefaultTitle = ( val ) => {
			...
			tsfTitle.updateStateFor( _titleId, 'defaultTitle', defaultTitle );
		}
	...
	const _initDescriptionListeners = () => {
		...
		tsfDescription.updateStateFor( _descId, 'allowReferenceChange', ! state.refDescriptionLocked );
		tsfDescription.updateStateFor( _descId, 'defaultDescription', tsf.escapeString( state.defaultDescription.trim() ) );
		tsfDescription.updateStateFor( _descId, 'hasLegacy', !! ( state.hasLegacy || false ) );
		...
		const onSuccess = response => {
			...
			switch ( response.type ) {
				case 'success':
					...
					setTimeout( () => {
						if ( tsfDescription ) {
							tsfDescription.updateStateFor( _descId, 'defaultDescription', tsf.escapeString( response.data.metadescription.trim() ) );
						}
						if ( tsfSocial ) {
