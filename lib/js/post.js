...
window.tsfPost = function( $ ) {
	 ...
	 * @type {string}
	 */
	const _ogTitleId = 'autodescription_og_title';
	/**
	 * @since 4.2.0
	 * @access private
	 * @type {string}
	 */
	const _ogDescId = 'autodescription_og_description';
	 ...
	 * @type {string}
	 */
	const _twTitleId = 'autodescription_twitter_title';
	/**
	 * @since 4.2.0
	 * @access private
	 * @type {string}
	 */
	const _twDescId = 'autodescription_twitter_description';
	...
	const _initDescriptionListeners = () => {
		...
		const classicVisibilityInput = document.querySelector( '#visibility .save-post-visibility' );
		classicVisibilityInput && classicVisibilityInput.addEventListener( 'click', setClassicDescriptionVisibilityPrefix );
	...
	const _initSocialListeners = () => {
		...
		let ogTitleState = JSON.parse(
			document.getElementById( `tsf-og-title-data_${_ogTitleId}` ).dataset.state
		);
		let twTitleState = JSON.parse(
			document.getElementById( `tsf-tw-title-data_${_twTitleId}` ).dataset.state
		);
		let ogDescState = JSON.parse(
			document.getElementById( `tsf-og-description-data_${_ogDescId}` ).dataset.state
		);
		let twDescState = JSON.parse(
			document.getElementById( `tsf-tw-description-data_${_twDescId}` ).dataset.state
		);

		tsfSocial.updateState( 'useTagline', ogTitleState.addAdditions ); // twTitleState also has this.
		tsfSocial.updateState( 'ogTitleDefault', ogTitleState.defaultTitle );
		tsfSocial.updateState( 'twTitleDefault', twTitleState.defaultTitle );
		tsfSocial.updateState( 'ogDescDefault', ogDescState.defaultDesc );
		tsfSocial.updateState( 'twDescDefault', twDescState.defaultDesc );
		...
		tsfSocial.initTitleInputs( {
			...
			og:    document.getElementById( _ogTitleId ),
			ogRef: document.getElementById( `tsf-og-title-reference_${_ogTitleId}` ),
			tw:    document.getElementById( _twTitleId ),
			twRef: document.getElementById( `tsf-tw-title-reference_${_twTitleId}` ),
		} );
		...
		tsfSocial.initDescriptionInputs( {
			...
			og:    document.getElementById( _ogDescId ),
			ogRef: document.getElementById( `tsf-og-description-reference_${_ogDescId}` ),
			tw:    document.getElementById( _twDescId ),
			twRef: document.getElementById( `tsf-tw-description-reference_${_twDescId}` ),
		} );
	}
	...
	const _initUpdateMetaBox = () => {
		...
		const getData = {
			...
			ogdescription:   !! document.getElementById( _ogDescId ),
			twdescription:   !! document.getElementById( _twDescId ),
			imageurl:        !! imageUrl,
		};
				...
				case 'success':
					...
					setTimeout( () => {
						...
						if ( 'tsfSocial' in window ) {
							// tsfSocial.updateState( 'ogTitleDefault', response.data.ogTitle.trim() );
							// tsfSocial.updateState( 'ogDescDefault', response.data.twTitle.trim() );
							tsfSocial.updateState( 'ogDescDefault', response.data.ogdescription.trim() );
							// tsfSocial.updateStateOf( _ogDescId, 'ogDescDefault', response.data.ogdescription.trim() );
							tsfSocial.updateState( 'twDescDefault', response.data.twdescription.trim() );
							// tsfSocial.updateStateOf( _twDescId, 'twDescDefault', response.data.twdescription.trim() );
						}
