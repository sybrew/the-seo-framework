	...
	function _initVisibilityListeners() {
		...
		if ( canonicalInput ) {
			...
			if ( writePostname ) {
				...
				postSlugs = state.parentPostSlugs.map( post => post.slug ); // isHierarchical is checked in PHP for this.
			}
			...
			const updateCanonical = () => {
				if ( writePostname ) {
					...
					const postName = tsfCanonical.sanitizeSlug( activeSlug.substring( 0, 200 ) );

					// We rewrote %pagename% to %postname% at `Meta\URI\Utils::get_url_permastruct()`
					urlDataParts.set( `%postname%`, [ ...postSlugs, postName ].join( '/' ) );
				}
			...
			document.addEventListener(
				...
				async event => {
					...
					switch ( event.detail.type ) {
						...
						case 'parent':
							if ( writePostname ) {
								postSlugs = await tsfPostSlugs.get( event.detail.postData.get( 'parent' ) );
								queueUpdateCanonical();
							}
							break;
						case 'author':
				...
				if ( writePostname ) {
					...
					if ( parentIdInput ) {
						const updateParentSlug = tsfUtils.debounce(
							async () => {
								postSlugs = await tsfPostSlugs.get( parentIdInput.value );
								queueUpdateCanonical();
							},
							100, // Magic number. High enough to prevent self-DoS, low enough to be responsive.
						);
