...
window.tsfPTLE = function () {
	...
	function _injectSelector( taxonomySlug, postId, currentValue ) {
		...
		const taxonomyWrap = document.querySelector( `#edit-${postId} #taxonomy-${taxonomySlug}` );

		if ( ! taxonomyWrap ) return;

		// Check if already injected
		if ( taxonomyWrap.querySelector( `.tsf-pt-le-selector-${taxonomySlug}` ) ) return;
		...
		const checklist = taxonomyWrap.querySelector( `#${taxonomySlug}checklist, #${taxonomySlug}-all` );
		if ( checklist ) {
			checklist.parentNode.insertBefore( selectorWrap, checklist.nextSibling );
		} else {
			taxonomyWrap.appendChild( selectorWrap );
		}
	...
	function _populateSelector( taxonomySlug, postId, currentValue ) {
		...
		const checkboxes = document.querySelectorAll(
			`#edit-${postId} #${taxonomySlug}checklist input[type=checkbox]:checked, ` +
			`#edit-${postId} #${taxonomySlug}-all input[type=checkbox]:checked`,
		);
	...
	function _initBulkEdit() {
		...
		// Bulk edit is a single form, not per-post
		const bulkEditRow = document.getElementById( 'bulk-edit' );

		if ( ! bulkEditRow ) return;

		for ( const taxonomySlug in supportedTaxonomies ) {
			const taxonomyWrap = bulkEditRow.querySelector( `.inline-edit-col .cat-checklist#${taxonomySlug}checklist` )?.closest( '.inline-edit-categories' );

			if ( ! taxonomyWrap ) continue;

			// Check if already injected
			if ( taxonomyWrap.querySelector( `.tsf-pt-le-selector-${taxonomySlug}` ) ) continue;
			...
			const checkboxes = taxonomyWrap.querySelectorAll( `input[type=checkbox]` );
