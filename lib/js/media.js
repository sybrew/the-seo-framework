			...
			const writeTooltips = success => {
				...
				if ( imageWarning ) {
					...
					if ( ! warning.length ) {
						...
						if ( ext?.length ) {
							// Check for warned types: first imageType-specific, then 'all'
							if ( l10n.warning.warnedTypes?.[ imageType ]?.hasOwnProperty( ext )
								|| l10n.warning.warnedTypes?.all?.hasOwnProperty( ext )
							) {
								warning = l10n.warning.i18n.extWarned.replace( '%s', tsf.escapeString( ext ) );
							}
							// Check for forbidden types: first imageType-specific, then 'all'
							else if ( l10n.warning.forbiddenTypes?.[ imageType ]?.hasOwnProperty( ext )
