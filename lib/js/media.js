...
window.tsfMedia = function( $ ) {
	...
	const calculateImageSelectOptions = ( attachment, controller ) => {
		...
		// This is taken from WordPress' very own 'calculateImageSelectOptions' as-is.
	...
	const resetImageEditorSetActions = () => {
		$( '.tsf-set-image-button' ).off( 'click', _openImageEditor );
		$( '.tsf-set-image-button' ).on( 'click', _openImageEditor );
	}
	...
	const resetImageEditorRemovalActions = () => {
		$( '.tsf-remove-image-button' ).off( 'click', _removeEditorImage );
		$( '.tsf-remove-image-button' ).on( 'click', _removeEditorImage );
	}
