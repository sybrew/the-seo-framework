...
window.tsfMedia = function () {
	...
	function _updateImageNotifications( event ) {
		...
		const preview      = document.getElementById( `${imageId}-preview` ),
			  ...
			  selectButton = document.getElementById( `${imageId}-select` ),
			  inputType    = selectButton?.dataset?.inputType || '';
		...
		const updateToolTip = () => {
			...
			const writeTooltips = success => {
				...
				if ( imageWarning ) {
					...
					if ( ! warning.length ) {
						// Only show extension warnings for social images, not for logos (which may not apply to all contexts)
						if ( inputType === 'social' ) {
