...
window.tsfTT = function( $ ) {
	...
	const triggerUpdate = element => {
		if ( ! element || ! ( element instanceof Element ) ) element = document.querySelector( toolTipItemSelector );
		element && element.dispatchEvent( new CustomEvent( 'tsf-tooltip-update' ) );
	}
