...
window.tsfTT = function( $ ) {
	...
	const _initToolTips = () => {
		...
		addBoundary( '#wpcontent' ); //! All pages, but Gutenberg destroys the boundaries.. @see tsfGBC
	}
	 ...
	 *
	 * @since 3.1.0
	 * @since 3.3.0 Tooltips are now prepended, instead of appended--so they no longer break the order of flow.
	...
	const doTooltip = ( event, element, desc ) => {
		...
		let hoverItemWrapWidth = $hoverItemWrap.width();
		...
		let textWrapWidth   = $textWrap.outerWidth(),
			textWidth       = $innerText.outerWidth(),
