...
window.tsfTT = function( $ ) {
	...
	const _initToolTips = () => {
		...
		const setTouchBuffer = () => {
			...
			touchBuffer = setTimeout( () => {
				inTouchBuffer = false;
			}, 300 ); // Click attenuation guess: <https://patrickhlauke.github.io/touch/tests/results/>, let's do 300.
		}
		...
		const requestMoveMoveAnimation = () => {
		...
		const doMouseMove = () => {
			let isMouseEvent = ! [ mouse.curr.x ].includes( NaN );
			if ( isMouseEvent ) {
				if ( mouse.curr.x === mouse.last.x ) {
					requestMoveMoveAnimation();
					return;
				}
			}

			mouse.last.x = mouse.curr.x;

			const event = lastAnimationEvent;

			let	tooltip = activeTooltipElements.tooltip || ( event && event.target.querySelector( '.tsf-tooltip' ) );
			...
			if ( ! activeTooltipElements.wrap ) {
				activeTooltipElements.wrap = event.target.closest( '.tsf-tooltip-wrap' ) || event.target.parentNode;
			}

			let pagex         = mouse.curr.x,
				arrowBoundary = 7,
				arrowWidth    = 16;
		...
		const touchRemove = event => {
			...
			if ( event.target.classList.contains( 'tsf-tooltip-item' ) ) {
				balloonToKeep = event.target;
			}
			if ( ! balloonToKeep ) {
				let children = event.target.querySelectorAll( itemSelector );
				for ( const element of children ) {
					if ( element.querySelector( balloonSelector ) ) {
						balloonToKeep = element;
						break;
					}
				}
			}
			...
			if ( balloonToKeep ) {
				...
				for ( const element of document.querySelectorAll( itemSelector ) ) {
					if ( element !== balloonToKeep ) {
		...
		const loadToolTip = event => {
			...
			if ( isTouch ) {
				...
				// Click attenuation guess... It's a guess. Can we make it educated?
				// This seems nice: <https://patrickhlauke.github.io/touch/tests/results/>, let's do 300.
				// setTimeout( () => event.target.dataset.preventedClick = 1, 300 );
				// This guess is highly annoying during normal usage. Let's just set it and rely on the buffer.
				event.target.dataset.preventedClick = 1;
			}
			cancelMouseMoveAnimation();
		 ...
		 *
		 * FIXME: When a tooltip is injected via another script (via tsfTT.doTooltip()) that has the tsf-tooltip-wrap class,
		...
		const preventTooltipHandleClick = event => {
			if ( event.target.dataset.preventedClick ) return;
			event.preventDefault();
			// iOS 12 bug causes two clicks at once. Let's set this asynchronously.
			setTimeout( () => event.target.dataset.preventedClick = 1 );
		}
		...
		const options = passiveSupported ? { capture: true, passive: true } : true;
		/**
		 * Initializes tooltips.
		 * @function
		 */
		const init = () => {
			...
			initTimeout = setTimeout( () => {
				...
				for ( let i = 0; i < wraps.length; i++ ) {
					...
					wraps[i].addEventListener( 'click', preventTooltipHandleClick );
				}
	 ...
	 */
	const getTooltip = element => $( element ).find( '.tsf-tooltip' ).first();
