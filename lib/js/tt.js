...
window.tsfTT = function() {
	...
	const _renderTooltip = ( event, element, desc ) => {
		...
		const boundary      = element.closest( ttSelectors.boundary ) || document.body,
		...
		const textWrapWidth   = textWrap.offsetWidth,
		...
		// If there's a super-wrap, make everything relative from the top-left corner.
		if ( hoverItemSuperWrap ) {
			const superRect = hoverItemSuperWrap.getBoundingClientRect();
			// This is basically hoverItemWrapRect.offsetTop/offsetLeft but then with subpixel calculations.
			offsetTop  = hoverItemWrapRect.top - superRect.top,
			offsetLeft = hoverItemWrapRect.left - superRect.left;
			...
			offsetTop -= superRect.height - hoverItemWrapRect.height;
		}

		tooltip.style.left     = `${horIndent + offsetLeft}px`;
		tooltip.dataset.adjust = horIndent; // This is relative to .left; don't add offset.
