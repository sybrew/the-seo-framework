...
window.tsfTT = function( $ ) {
	...
	const _initToolTips = () => {
		...
		// TODO debounce this via requestAnimationFrame? That might solve the EdgeHTML bottleneck.
		// Beware of race conditions, though; however, the first if-statement resolves that?
		// e.g. : register mouse positions, enable ticker, request animation frame loop (keep noting latest positions), disable ticker once mouse stops.
