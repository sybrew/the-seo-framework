...
window.tsfTT = function( $ ) {
	 ...
	 *
	 * @since 3.1.0
	 * @since 3.3.0 Now adds default boundary to `wpwrap` instead of `wpcontent`.
	 * @access private
	 *
	 * @function
	...
	const _initToolTips = () => {
		...
		const loadToolTip = ( event ) => {
			...
			mouseEnter( event );
			//= Initiate arrow placement directly for Windows Touch or when overflown.
			mouseMove( event );
	 ...
	 *
	 * @since 3.1.0
	 * @since 3.3.0 1. Tooltips are now prepended, instead of appended--so they no longer break the order of flow.
	...
	const doTooltip = ( event, element, desc ) => {
		...
		let $tooltip = $(
			`<div class="tsf-tooltip"><span class="tsf-tooltip-text-wrap"><span class="tsf-tooltip-text">${desc}</span></span><div class="tsf-tooltip-arrow"></div></div>`
		);
		$hoverItem.prepend( $tooltip );

		let $boundary = $hoverItem.closest( '.tsf-tooltip-boundary' );

		$boundary = $boundary.length && $boundary || $( document.body );

		let boundaryTop   = $boundary.offset().top - ( $boundary.prop( 'scrolltop' ) || 0 ),
			boundaryWidth = $boundary.outerWidth(),
			maxWidth      = 250,
			appeal        = 12;
		...
		if ( $textWrap.width() > maxWidth && hoverItemWrapWidth < maxWidth && hoverItemWrapWidth > 150 ) {
			// The hoveritemwrap is of an acceptable size. Format it to that.
			$textWrap.css( 'flex-basis', hoverItemWrapWidth + 'px' );
		}

		// Calculate the appeal with the spacing.
		if ( $textWrap.outerWidth() > ( boundaryWidth - ( appeal / 2 ) ) ) {
			// Resize if it overflows the boundary size, maximum 250px.
			$textWrap.css( 'flex-basis', Math.min( maxWidth, boundaryWidth - appeal ) + 'px' );
			// Full appeal has been added, adjust that accordingly for either side.
			appeal = appeal / 2;
		} else if ( $textWrap.width() > maxWidth ) {
			// Resize the text wrap if it exceeds 250px on auto-grow.
			$textWrap.css( 'flex-basis', maxWidth + 'px' );
		}

		let boundaryLeft  = $boundary.offset().left - ( $boundary.prop( 'scrollLeft' ) || 0 ),
			boundaryRight = boundaryLeft + boundaryWidth;

		let textWrapWidth   = $textWrap.outerWidth(),
			textBorderLeft  = $textWrap.offset().left,
		...
		if ( textBorderLeft < boundaryLeft ) {
			...
			horIndent = boundaryLeft - textBorderLeft + 12;
			...
			horIndent = boundaryRight - textBorderLeft - textWrapWidth - 12;
			...
			//= Wrap is larger than tooltip. Find middle of pointer and adjust accordingly.
			...
			if ( left < 0 ) {
				...
				horIndent = hoverItemWrapWidth - textWrapWidth;
			}
		}

		if ( ( horIndent + textBorderLeft ) < ( boundaryLeft + appeal ) ) {
			let _adjustLeft = ( horIndent + textBorderLeft ) - ( boundaryLeft + appeal );
			horIndent = horIndent - _adjustLeft;
		}
		if ( ( horIndent + textBorderRight ) > ( boundaryRight + appeal ) ) {
			let _adjustRight = ( horIndent + textBorderRight ) - ( boundaryRight + appeal );
