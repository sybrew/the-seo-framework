...
window.tsfTT = function () {
	...
	const _activeToolTipHandles = {
		updateDesc: event => {
			if ( ! event.target.matches( ttSelectors.item ) ) return;
			const tooltip = ttMap.get( event.target );
			if ( ! tooltip ) return;

			const tooltipText = tooltip.querySelector( ttSelectors.text );
	...
	const {
		...
		const animate = () => {
			...
			let mousex        = pagex - wrapRect.left - ( arrowWidth / 2 ),
