...
window.tsfTT = function( $ ) {
	...
	const toolTipTextName         = 'tsf-tooltip-text';
	const toolTipTextSelector     = `.${toolTipTextName}`;
	const toolTipTextWrapName     = 'tsf-tooltip-text-wrap';
	const toolTipTextWrapSelector = `.${toolTipTextWrapName}`;
	const toolTipBoundaryName     = 'tsf-tooltip-boundary';
	const toolTipBoundarySelector = `.${toolTipBoundaryName}`;
	const toolTipArrowName        = `tsf-tooltip-arrow`;
	const toolTipArrowSelector    = `.${toolTipArrowName}`;

	const _activeToolTipHandles = {
		updateDesc: event => {
			if ( event.target.classList.contains( toolTipItemName ) ) {
				let tooltipText = event.target.querySelector( toolTipTextSelector );
		...
		pointerLeave: event => {
			...
			_pointerAnimation.cancelMouseMoveAnimation();
		}
	}
	...
	const _events = target => {
		...
		return {
			set: () => {
				...
				// Always set this, as the events may be reintroduced via other code. var_dump() was this a bug?
			...
			unset: () => {
				...
				// Always set this, as the events may be reintroduced via other code. Was this a bug?
				// target.addEventListener( 'tsf-tooltip-update', _activeToolTipHandles.updateDesc );
			},
		};
	}
	...
	const _pointerAnimation = {
		_pointerAnimationId:       void 0,
		requestMouseMoveAnimation: () => {
			_pointerAnimation._pointerAnimationId = requestAnimationFrame( _doMouseMove );
		},
		cancelMouseMoveAnimation:  () => {
			_pointer.lastMoveEvent = void 0;
			_activeTooltipElements.reset();
			_pointer.reset();
			cancelAnimationFrame( _pointerAnimation._pointerAnimationId );
		}
	}
	...
	const _doMouseMove = () => {
		...
		if ( isMouseEvent ) {
			if ( _pointer.currPos.x === _pointer.lastPos.x ) {
				_pointerAnimation.requestMouseMoveAnimation();
				return;
			}
		}
		...
		let	tooltip = _activeTooltipElements.tooltip || ( event && event.target.querySelector( toolTipSelector ) );
		...
		_activeTooltipElements.arrow   ||= tooltip.querySelector( toolTipArrowSelector );
		_activeTooltipElements.wrap    ||= event.target.closest( toolTipWrapSelector ) || event.target.parentNode;
		...
		let mousex        = pagex - _activeTooltipElements.wrap.getBoundingClientRect().left - ( arrowWidth / 2 ),
			textWrap      = _activeTooltipElements.tooltip.querySelector( toolTipTextWrapSelector ),
		...
		if ( adjust ) {
			...
			if ( boundaryRight + adjust > _activeTooltipElements.wrap.offsetWidth ) {
				let innerText = textWrap.querySelector( toolTipTextSelector ),
		...
		if ( isMouseEvent ) {
			...
			_pointerAnimation._pointerAnimationId && _pointerAnimation.cancelMouseMoveAnimation();
		}
	}
	 ...
	 * @param {Event} event
	 */
	const _loadToolTip = async ( event ) => {
		...
		const init = () => {
			...
			initTimeout = setTimeout( () => {
				...
				for ( let i = 0; i < wraps.length; i++ ) {
					...
					// We mitigated this issue at _loadToolTip().
			...
			}, 25 ); // Increase this for slow computers? Or allow the loop to be broken from the outside? -> Generators.
		}
		init();
	 ...
	 */
	const doTooltip = async ( event, element, desc ) => {
		...
		for ( const element of document.querySelectorAll( toolTipSelector ) ) {
		...
		tooltip.insertAdjacentHTML(
			'afterbegin',
			`<span class=${toolTipTextWrapName}><span class=${toolTipTextName}>${desc}</span></span><div class=${toolTipArrowName} style=will-change:left></div>`
		);

		element.prepend( tooltip );

		let boundary      = element.closest( toolTipBoundarySelector ) || document.body,
		...
		let hoverItemWrap      = element.closest( toolTipWrapSelector ) || element.parentElement,
			...
			textWrap           = tooltip.querySelector( toolTipTextWrapSelector ),
	 ...
	 * @return {undefined}
	 */
	const addBoundary = element => element instanceof Element && element.classList.add( toolTipBoundaryName );
	...
	const getTooltip = element => {
		...
		return element?.classList.contains( toolTipName )
			? element
			: element?.querySelector( toolTipSelector );
	}
