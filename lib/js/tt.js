...
window.tsfTT = function () {
	...
	const {
		...
		const animate = () => {
			...
			const { tooltip, arrow, wrap } = _activeTooltipElements.get( element );
			...
			if ( 'focus' === event.type ) {
				...
				pagex = tooltip.dataset.lastPagex || element.getBoundingClientRect().left;
			}
			...
			let mousex        = pagex - wrap.getBoundingClientRect().left - ( arrowWidth / 2 ),
			...
			if ( adjust ) {
				...
				if ( boundaryRight + adjust > wrap.offsetWidth ) {
	...
	function removeTooltip( element ) {
		...
		const toolTip = getTooltip( element );
		// Remove the tooltip directly only if contained. In Gutenberg, the override handles this.
		if ( element.contains( toolTip ) ) {
			toolTip?.parentNode.removeChild( toolTip );
		}
		ttMap.delete( element );
	}
	...
	function GBCTooltips() {
		const { createElement: h, useEffect, useState } = wp.element;
		const { Popover } = wp.components;

		const [ [ element, desc, tooltipCallback ] = [], tip ] = useState();
		useEffect( () => {
			GBCTooltips.tip = tip;
		}, [] );
		if ( ! desc ) return;
		...
		return h( Popover,
			{
				// Key to force recreating the popover. Otherwise reading the popover’s rectangle is
				// stale when the new tootip’s wrap was adjacent to the prior one’s. This likely can
				// be obviated given the idea is to stop using state and render to the root directly.
				key: desc,
				ref: ( popover ) => {
					if ( ! popover ) return;
					tooltipCallback( popover );
					// The bounding rectangle of the Popover component won’t be available immediately
					// - at least, most of the time - so wait a frame.
					requestAnimationFrame( () => {
						const { x: referenceX } = element.getBoundingClientRect();
						const { x: popoverX } = popover.getBoundingClientRect();
						const relativeX = referenceX - popoverX;
						popover.lastElementChild.style.marginLeft = `${ relativeX }px`
					} );
				},
	...
	function doGBC() {
		const { createRoot, createElement: h } = wp.element;
		const rootNode = document.createElement( 'div' );
		rootNode.classList.add( 'tsf-gbc-tooltip-root' );
		const root = createRoot(
			document.body.appendChild( rootNode )
		);
		root.render( h( GBCTooltips ) );

		ttSelectors.arrow = '.components-popover__arrow';

		_renderTooltip = async ( _event, element, desc ) => {
			element.dataset.hasTooltip = 1;
			const tooltipPromise = new Promise( ( resolve ) => {
				const ref = ( tooltip ) => resolve( tooltip );
				// root.render( h( Popover, { ref } ) );
				GBCTooltips.tip( [ element, desc, ref ] )
			} );
			return await tooltipPromise;
		}

		const baseRemoveTooltip = removeTooltip;
		removeTooltip = ( element ) => {
			GBCTooltips.tip();
			baseRemoveTooltip( element );
		}
	}
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'tsf-ready', () =>{
				_initToolTips();
				if ( typeof window.tsfGBC !== 'undefined' ) doGBC();
			} );
		},
	}, {
		/**
