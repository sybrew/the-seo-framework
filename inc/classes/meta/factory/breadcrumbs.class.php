...
class Breadcrumbs {
	...
	private static function get_singular_breadcrumb_list( $id = 0 ) {
		...
		if ( \is_post_type_hierarchical( $post_type ) ) { // page.
			...
			foreach ( array_reverse( $post->ancestors ) as $ancestor_id ) {
				$crumbs[] = [
					'url'  => Factory\URI::get_bare_singular_canonical_url( $ancestor_id ),
					'name' => Factory\Title::get_bare_title( [ 'id' => $ancestor_id ] ),
				];
			}
		} else { // single.
			...
			if ( $taxonomy ) {
				...
				$crumbs[] = [
					'url'  => Factory\URI::get_bare_post_type_archive_canonical_url( $post_type ),
					'name' => Factory\Title::get_bare_title( [ 'pta' => $post_type ] ),
				];
			}
		}

		$crumbs[] = [
			'url'  => Factory\URI::get_bare_singular_canonical_url( $post->ID ),
			'name' => Factory\Title::get_bare_title( [ 'id' => $post->ID ] ),
		];
	...
	private static function get_archive_breadcrumb_list( $object = null ) {
		...
		if ( null === $object ) {
			if ( Query::is_editable_term() ) {
				...
				$crumbs[] = [
					'url'  => Factory\URI::get_bare_taxonomical_canonical_url(),
					'name' => Factory\Title::get_bare_title(),
				];
			} elseif ( \is_post_type_archive() ) {
				$crumbs[] = [
					'url'  => Factory\URI::get_bare_post_type_archive_canonical_url( $object->name ),
					'name' => Factory\Title::get_bare_title(),
				];
			} elseif ( Query::is_author() ) {
				$crumbs[] = [
					'url'  => Factory\URI::get_bare_author_canonical_url(),
					'name' => Factory\Title::get_bare_title(),
				];
			} elseif ( \is_date() ) {
				...
				$crumbs[] = [
					'url'  => Factory\URI::get_bare_date_canonical_url( $year, $month, $day ),
					'name' => Factory\Title::get_bare_title(),
				];
			}
			...
			$crumbs[] = [
				'url'  => Factory\URI::get_post_type_archive_canonical_url( $object->name ),
				'name' => Factory\Title::get_bare_title( [
					'pta' => $object->name,
				] ),
			];
	...
	private static function get_search_breadcrumb_list() {
		return [
			static::get_front_breadcrumb(),
			[
				'url'  => Factory\URI::get_search_canonical_url(),
				'name' => Factory\Title::get_search_query_title(),
			],
		];
	}
	...
	private static function get_404_breadcrumb_list() {
		return [
			...
			[
				'url'  => '',
				'name' => Factory\Title::get_404_title(),
			],
		];
	}
	...
	private static function get_front_breadcrumb() {
		return [
			...
			'name' => Factory\Title::get_front_page_title(),
		];
	}
}
