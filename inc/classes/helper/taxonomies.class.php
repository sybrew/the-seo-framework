...
/**
 * Holds a collection of helper methods for Taxonomies.
 *
 * @since 4.3.0
 * @access protected
 *         Use tsf()->taxonomies() instead.
 */
class Taxonomies {
	 ...
	 */
	public static function is_taxonomy_disabled( $taxonomy = null ) {
		...
		if ( $taxonomy && Data\Plugin::get_option( 'disabled_taxonomies', $taxonomy ) ) {
			...
			foreach ( static::get_post_types_from_taxonomy( $taxonomy ) as $type ) {
				if ( Post_Types::is_post_type_supported( $type ) ) {
					$disabled = false;
					break;
				}
				$disabled = true;
	...
	public static function is_taxonomy_supported( $taxonomy = '' ) {
		...
		return (bool) \apply_filters_ref_array(
			...
			[
				$taxonomy
					&& ! static::is_taxonomy_disabled( $taxonomy )
					&& \in_array( $taxonomy, static::get_public_taxonomies(), true ),
				$taxonomy,
			]
		);
	}
	...
	public static function get_supported_taxonomies() {
		return memo() ?? memo(
			array_values(
				array_filter(
					static::get_public_taxonomies(),
					[ static::class, 'is_taxonomy_supported' ]
				)
			)
		);
	}
			...
			?? umemo(
				...
				\apply_filters(
					...
					array_filter(
						array_unique(
							array_merge(
								static::get_forced_supported_taxonomies(),
	 ...
	 */
	public static function get_forced_supported_taxonomies() {
		/**
		 * @since 4.1.0
	 ...
	 */
	public static function get_hierarchical_taxonomies_as( $get = 'objects', $post_type = '' ) {
	 ...
	 */
	public static function get_taxonomy_label( $tax_type, $singular = true ) {
