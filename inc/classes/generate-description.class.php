...
class Generate_Description extends Generate {
	 ...
	 * @since 3.0.6 Silently deprecated.
	 * @since 3.1.0 1. Removed the redundant preview check.
	 *              2. Removed caching.
	 * @deprecated Use `get_generated_description()` instead.
	 *
	 * @param array $args description args : {
	...
	protected function generate_the_description( $args, $escape = true ) {
		...
		if ( $args['social'] ) {
			...
			$description = $this->get_description_excerpt_social( $args['id'], $term ) ?: '';
		} else {
			$description_data = $this->get_description_excerpt_normal( $args['id'], $term );

			if ( $description_data['excerpt'] ) {
				if ( $description_data['trim'] ) {
					$description = $description_data['excerpt'];
				} else {
					...
					$description = sprintf(
						\_x( '%1$s %2$s %3$s', '1: Title, 2: Separator, 3: Excerpt', 'autodescription' ),
						$title_on_blogname,
						$sep,
						$description_data['excerpt']
					);
				}
			} else {
