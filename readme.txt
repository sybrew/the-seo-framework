...
In this minor update, we bring you the most advanced description generation yet. The generator is now context-sensitive, so you can expect the descriptions to be even more natural; a true time-saver! Oh, we also fixed some bugs, and we've implemented some improvements in preparation for version 3.3.0.
...
**For everyone:**
	...
	* **Improved:**
		* Google can now respond to the `noindex` directive quicker by removing the canonical URL.
...
**For developers:**
	...
	* **Method notes:**
		...
		* **Changed:**
			...
			* `page()` and `page()` now point to the correct page if the page number doesn't exist, and they now always return `1` on the admin screens to prevent excessive processing and unpredictable behavior of methods that rely on these.
