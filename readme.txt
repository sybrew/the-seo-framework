...
If you however require a more expanded sitemap, feel free to activate a dedicated sitemap plugin. The SEO Framework will deactivate its sitemap functionality when another (known) sitemap plugin is found.
...
**For everyone:**
/
* Added: Pinterest Webmaster verification field.
* Added: Feed option in the new Feed Settings metabox tab to convert the feed content into to an excerpt (400 characters) to counter scrapers and to prevent duplicated content issues.
* Added: Feed option to add a non-followed backlink to the feed content to counter scrapers and bad links.
* Added: PHP 5.2 compatibility, which is actually very much outdated. PHP 5.2 will henceforth be (syntaxwise) supported until at least March 2016. TODO test each file.
* Added: Subtle sitemap plugin detection notification on SEO Settings Page.
...
* MAYBE TODO Improved: The AutoDescription generation will also look at title tags. These title tags and their content will be excluded. TODO use preg_replace
...
* Improved: All archives get the archive title correctly. TODO TEST THIS
* Improved: All single sentence html hover titles have no more dots at the end to maintain consistency.
* Improved: Added back the bottom border on navigation tabs in the SEO Settings screen for mobile devices (got removed in WordPress core, probably by mistake).
* Improved: Navigation tabs in the SEO Setting page now don't overflow anymore in the Social Meta Settings metabox.
* Improved: Greatly reduced non-minified CSS size by converting spaces into tabs.
...
* TODO Fixed: Trimming of spaces around the blogname part of the title.
* TODO Fixed: og:locale when WPML is active (it/en -> it_IT en_US).
* Fixed: Saving Draft after SEO Settings change gave an unecessary leave page popup notification.
...
* Fixed: WPML Negatiation Type miscalculation, Different Domain Language URL format was treated as the subdirectory URL format.
...
* TODO Fixed: Another `wp_title()` bug. TODO See woocommerce page. TODO verify.
* TODO Updated: POT file. // Because of new text lines in the Pinterest/Sitemap notification.
* Note: With CSS, The SEO Framework will only actively support the latest version of WordPress, as it's very difficult to maintain and compare differences. Luckily, the worst that can happen is that a pixel is misplaced in the admin area.
...
* TODO Added: More filterable title arguments to disable sanitation.
...
* Added: With up to 140 calls in a single load in the admin area, a new cached function has arrived: `AutoDescription_PostData::get_blogname()`. Effectively reduced RAM usage by 80kB!
* Added: New cached function: `AutoDescription_PostData::get_blogdescription()`.
* Added: New cached function: `AutoDescription_Detect::can_i_use()`. Dump your multidimensional array of functions, classes and constants in this function and it will calculate if they can all be used and caches and matches them for these checks are quite intensive.
...
**TEMP LOG**
/
* NOTE ADDED: the_seo_framework_pintsite_output string filter
