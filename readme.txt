...
* **Other:**
	* Are you having difficulties translating some sentences, namely with gender types? [Let me know](https://github.com/sybrew/the-seo-framework/issues/new)!
		* You can use label `i18n/i10n` for these issues.

**For developers:**

* **Added:**
	* Method `s_title_raw`, it is a copy of `escape_title` and is used instead of `s_title` for Home Page SEO title and tagline sanitation.
	* Method `s_nbsp`, translates space entities to spaces.
	* Method `s_dupe_space`, removed duplicated spaces.
	* Method `s_bsol`, translates backslashes to entities.
	* Method `s_tabs`, translates tabs to spaces, one tab to one space.
	* Method `s_singleline`, converts multiline to single line with spaces in between.
	* Method `s_title_raw`, used for database and internal sanitation.
	* Method `s_description_raw`, used for database and internal sanitation.
	* Method `s_excerpt_raw`, used for database and internal sanitation.
* **Improved:**
	...
	* Method `get_image` can now perform on `$post->ID === 0`.
		* It will then skip featured image and custom social images.
* **Updated:**
	* Translation POT file.
* **Removed:**
	* **Warning:** `wp_kses_decode_entities` now no longer runs on the option functions.
		* This might cause some calculation functionality to have different effects.
		* Always be sure to sanitize your output.
	* The unused `googleplus_tags` boolean option has been removed.
		1. It's unused.
		1. With its removal we can auto-enable it easier in the future.
	* The unused `description_custom` string option has been removed.
		1. It's unused.
		1. With its removal we can auto-enable it easier in the future.

/
***I don't believe in luck, but I do believe in assigning [value to things](https://theseoframework.com/?p=TODO).***
