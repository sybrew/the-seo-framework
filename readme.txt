...
* **Added:**
	...
	* Time display options for the sitemap. Now you can select how the time is output. Default "Complete date plus hours, minutes and time zone".
	...
	* WooCommerce breadcrumbs support!
...
* **Improved:**
	...
	* Sanitation of URLs where query arguments must be removed is now compatible on any (odd) server configuration.
...
* **Added:**
	...
	* `AutoDescription_Generate_Title::get_the_real_archive_title()` function, which also works in the admin area and has a term object argument and outputs no HTML, effectively speeding the whole plugin up on archive pages.
...
* **Reworked:**
	...
	* `AutoDescription_TermData::get_term_filter()` no longer adds SEO data after the SEO output has been set (action `the_seo_framework_do_after_output`). This means the term SEO data might be unreachable afterwards when using `get_term()`.
* **Improved:**
	...
	* `The_SEO_Framework_Load::call_function()` now consumes less memory and has gained support for PHP 5.2.
...
* **Deprecated:**
	...
	* `AutoDescription_Siteoptions::initialize_defaults()` function. Without replacement.
* **Removed:**
...
* **Action Notes:**
	* **New:**
		...
		* `the_seo_framework_do_before_output`, not to be confused with the filter 'the_seo_framework_before_output'
		* `the_seo_framework_do_after_output`, not to be confused with the filter 'the_seo_framework_after_output'
* **Notes:**
